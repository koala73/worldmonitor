...
- **国家不安定指数 (CII)** — Tier-1 の 31 か国を対象としたサーバー権威型 CII v8 ストレススコアリング
...
すべてのサイトバリアントとデスクトップバイナリは単一のコードベースからビルドされ、同じリリースプロセスで配布されます。以下の表は、どの提供形態が安心して依存できるかを示すメンテナンス状況です。

| 提供形態 | ステータス | 備考 |
|---------|--------|-------|
| `worldmonitor.app`、`tech.`、`finance.`、`commodity.`、`happy.`、`energy.` | 安定版 | 本リポジトリからビルドされる公開デプロイ。積極的にメンテナンス中 |
| デスクトップバイナリ (Windows / macOS Apple Silicon / macOS Intel / Linux AppImage) | 安定版 | アプリ内でバリアントを切り替えられる単一の Tauri バイナリ。現在の CI リリース対象は `full` と `tech` |
...
| **API コントラクト** | Protocol Buffers (281 proto、35 サービス)、sebuf HTTP アノテーション |
