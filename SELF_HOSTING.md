...
docker compose pull && docker compose up -d        # or: uvx podman-compose pull && uvx podman-compose up -d
...
`docker compose pull` (above) fetches the three published images — you only need this if you're modifying the app itself or targeting a platform the published images don't cover.
...
# Full stack, built locally instead of pulled (docker-compose.yml keeps its
# build: sections for this)
...
- All three images use **Node.js 24 Alpine** for their builder stages
- Blog site build is skipped in Docker (separate dependencies)
- The `worldmonitor` runtime stage needs `gettext` (Alpine package) for `envsubst` in the nginx config
