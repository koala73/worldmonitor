...
| `dependabot-auto-merge.yml` | PR | Enables GitHub auto-merge on Dependabot's grouped `github-actions` pull request so it lands once the required checks pass, and only when all four guards hold: Dependabot is both the PR author and the pusher of the commit under review, the bump is minor or patch within the `github-actions` group, and every action in it is published by a GitHub- or Docker-owned org. A commit pushed to a Dependabot branch by anyone else actively disarms auto-merge. Requires **Allow auto-merge** in Settings -> General; without it the workflow goes red on each Dependabot PR and merges nothing. Every other ecosystem keeps a human: base-image bumps in particular, because they are what `docker-publish.yml`'s push-triggered rebuild then publishes |
