...
| `docker-publish.yml` | Release, weekly cron (Wed 02:19 UTC), manual | Multi-arch image (amd64, arm64) pushed to GHCR. The weekly rebuild is what bounds the published image's age, and `latest` follows `main` as well as a release: both `FROM` lines in `docker/Dockerfile` are digest-pinned, so a base fix reaches the registry only once Dependabot has moved the pin on `main` and something rebuilds from there. This is also the only workflow that builds `docker/Dockerfile` at all, so before any push it loads the amd64 image and requests its live routes; a failed scheduled run opens or re-comments a tracking issue, because nothing else watches a weekly cron |
