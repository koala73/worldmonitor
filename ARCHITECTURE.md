...
| `dependabot-auto-merge.yml` | PR | Enables GitHub auto-merge on Dependabot's grouped `github-actions` pull request so it lands once the required checks pass, and only when all four guards hold: Dependabot is both the PR author and the pusher of the commit under review, the bump is minor or patch within the `github-actions` group, and every action in it is published by a GitHub- or Docker-owned org. Any run whose commit fails a guard actively disarms an earlier arming, whether the commit came from someone else or from Dependabot refreshing the group with a major or an untrusted action, and arming is pinned to the head commit that was checked. Requires **Allow auto-merge** in Settings -> General; without it the workflow goes red on each Dependabot PR and merges nothing. Every other ecosystem keeps a human: base-image bumps in particular, because they are what `docker-publish.yml`'s push-triggered rebuild then publishes |
...
| `docker-publish.yml` | Release, push to main (docker-relevant paths), weekly cron (Wed 02:19 UTC, backstop), manual | Multi-arch image (amd64, arm64) pushed to GHCR. The push trigger is what actually bounds the published image's age in practice — a Dependabot base-image bump or an application change republishes `latest` within minutes of merging (action bumps under `.github/` do not trigger it and reach the image with the next rebuild), immune to GitHub's 60-day scheduled-workflow disablement; the cron is the backstop for what a push can miss. `latest` follows `main` as well as a release: both `FROM` lines in `docker/Dockerfile` are digest-pinned, so a base fix reaches the registry only once Dependabot has moved the pin on `main` and something rebuilds from there. This is also the only workflow that builds and pushes `docker/Dockerfile`'s image, so before any push it loads the amd64 image and requests its live routes; a failed push-triggered or scheduled run opens or re-comments a tracking issue, because nothing else watches an unattended publish |
