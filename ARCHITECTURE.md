...
| `dependabot-auto-merge.yml` | PR | Enables GitHub auto-merge on Dependabot's grouped minor/patch `github-actions` pull request so it lands once the required checks pass. Every other ecosystem and every major keeps a human: base-image bumps in particular, because they are what `docker-publish.yml`'s weekly rebuild then publishes |
