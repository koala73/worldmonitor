...
| `openrouter-free-models-live.yml` | Daily cron, push to main (model policy/suite paths), manual | Keyless check that both OpenRouter free-chain models in `scripts/lib/llm-model-policy.cjs` are still on the public `/models` listing at zero price; a delisted `:free` slug returns HTTP 404 on every call (#8570). Fails if fewer than 2 cases execute |
