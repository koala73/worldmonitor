...
The public dashboard exposes 56 map-layer types, 500+ curated news feeds backed by 534+ observed upstream hosts, country briefs, instability scores, chokepoints, infrastructure, markets, disasters, and watchlists. Every layer except the Resilience layer is available on the free plan, and the dashboard can be opened without an account.
...
Yes. World Monitor's public interface is generated from 297 Protocol Buffer definitions into 36 REST service specifications, alongside a Model Context Protocol server with 62 live tools. Public metadata and discovery surfaces are open; data-bearing API, MCP tool, and resource calls require the appropriate Pro/API OAuth session or API key.
