...
export const LAYER_REGISTRY: Record<keyof MapLayers, LayerDefinition> = {
  ...
  canadaAlerts:             def('canadaAlerts',             '&#9888;',   'canadaAlerts',             'Canada Alerts (AB + BC)', ['deck']),
...
export const LAYER_EXPLANATIONS: Partial<Record<keyof MapLayers, LayerExplanation>> = {
  ...
  natural: {
    ...
    related: ['Natural Events layer popups', 'US + Canada Weather Alerts (NWS, ECCC)', 'Country brief natural signals'],
  ...
  weather: {
    ...
    coverage: 'curated',
    category: 'Weather',
    purpose: 'Shows active severe-weather alerts for the United States and Canada from official national warning feeds.',
    source: 'United States National Weather Service (NWS) and Environment and Climate Change Canada (ECCC) active alerts, seeded through the WorldMonitor relay.',
    freshness: 'NWS and ECCC alerts are seeded every 15 minutes by the relay and monitored against a 45-minute freshness budget.',
    confidence: 'Authoritative for active NWS and ECCC alerts, subject to upstream publication timing and mapped alert geometry.',
    limitations: [
      'Does not include official national weather warnings outside the United States and Canada.',
  ...
  canadaAlerts: {
    ...
    purpose: 'Shows active Alberta and British Columbia public-safety warnings as map dots alongside US NWS weather.',
    source: 'Alberta Emergency Alert Atom and the OGL-BC Evacuation Orders and Alerts GeoJSON layer are normalized into one canadaAlerts union. CAP/GeoRSS polygons and B.C. evacuation polygons become centroids; records without geometry use the source province centroid.',
    freshness: 'Both provincial sources are seeded every 15 minutes and the union is monitored against a 45-minute freshness budget. An empty feed (no active alerts) is a valid zero-record success.',
    confidence: 'Authoritative for alerts published by Alberta Emergency Alert and B.C. Evacuation Orders and Alerts, subject to upstream publication timing and mapped alert geometry.',
    limitations: [
      'Coverage is limited to Alberta and British Columbia. Ontario has no eligible province-operated machine feed outside excluded NAAD, and SaskAlert redistribution rights are not established.',
      'Does not replace US NWS weather alerts or Ontario/Toronto road layers.',
      'Entries without a mappable severity (Alberta CAP/colour semantics or B.C. evacuation status) are dropped rather than invented.',
    ],
    related: ['US Weather Alerts (NWS)', 'Natural Events layer', 'Data freshness status'],
    evidence: ['scripts/seed-alberta-emergency-alert.mjs', 'scripts/seed-bc-emergency-info.mjs', 'api/health.js', 'src/services/canada-alerts.ts'],
  },
  flights: {
    key: 'flights',
    coverage: 'curated',
