...
const FULL_PANELS: Record<string, PanelConfig> = {
  ...
  'military-correlation': { name: 'Force Posture', enabled: true, priority: 2, ...(_desktop && { premium: 'locked' as const }) },
  'escalation-correlation': { name: 'Escalation Monitor', enabled: true, priority: 2, ...(_desktop && { premium: 'locked' as const }) },
  'economic-correlation': { name: 'Economic Warfare', enabled: true, priority: 2, ...(_desktop && { premium: 'locked' as const }) },
  'disaster-correlation': { name: 'Disaster Cascade', enabled: true, priority: 2, ...(_desktop && { premium: 'locked' as const }) },
