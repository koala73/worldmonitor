...
export class GlobeMap {
  ...
  private buildMarkerElement(d: GlobeMarker): HTMLElement {
    ...
    if (d._kind === 'conflict') {
      ...
      const hitScale = 2.5; // Hit target is 2.5x larger than visual dot
      el.innerHTML = `
        <div style="
          position:relative;
          width:${size * hitScale}px;height:${size * hitScale}px;
          display:flex;align-items:center;justify-content:center;
          margin-left:-${(size * hitScale) / 2}px;margin-top:-${(size * hitScale) / 2}px;
        ">
