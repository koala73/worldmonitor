...
export class CountryDeepDivePanel implements CountryBriefPanel {
  ...
  private renderEnergyProfile(data: CountryEnergyProfileData): void {
    ...
    if (data.jodiGasAvailable) {
      ...
      const gasLabel = this.el('span', '', `Gas imports: ${totalBcm} BCM/yr`);
    ...
    if (hasLiveSignals) {
      ...
      if (data.gasStorageAvailable) {
        ...
        row.textContent = `EU Gas Storage: ${data.gasStorageFillPct.toFixed(1)}% (${data.gasStorageTrend}) as of ${data.gasStorageDate}`;
        section.append(row);
      }
