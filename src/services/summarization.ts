 ...
 * Fallback: Ollama -> Atlas Cloud -> OpenRouter -> Groq -> Browser T5
 *
...
export type SummarizationProvider = 'ollama' | 'atlascloud' | 'groq' | 'openrouter' | 'browser' | 'cache';
...
export interface SummarizeOptions {
  skipCloudProviders?: boolean;  // true = skip Ollama/Atlas Cloud/Groq/OpenRouter, go straight to browser T5
...
// Order matches the server default chain for configured providers; the RPC
// honors the client-supplied provider, so the client's try-order decides which
// model summarizes.
const API_PROVIDERS: ApiProviderDef[] = [
  { featureId: 'aiOllama',      provider: 'ollama',     label: 'Ollama' },
  { featureId: 'aiAtlasCloud',  provider: 'atlascloud', label: 'Atlas Cloud' },
