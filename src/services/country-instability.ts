...
const ZONE_COUNTRY_MAP: Record<string, string[]> = {
  ...
  'Taiwan Strait': ['TW', 'CN'], 'Horn of Africa': ['SO', 'ET', 'ER', 'DJ'],
  'Sahel': ['ML', 'BF', 'NE', 'TD', 'MR', 'SD'], 'Central Africa': ['CF', 'CD', 'CM', 'CG'],
...
export function calculateCII(): CountryScore[] {
  ...
  for (const [code, name] of Object.entries(TIER1_COUNTRIES)) {
    ...
    const blendedScore = baselineRisk * 0.25 + eventScore * 0.75 + hotspotBoost + newsUrgencyBoost + focalBoost + displacementBoost + climateBoost + electionBoost;
