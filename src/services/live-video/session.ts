  ...
  const onMessage = (event: MessageEvent) => {
    ...
    switch (message.type) {
      ...
      case 'yt-error':
        ...
        break;
      case 'yt-mute-state':
        if (typeof message.muted === 'boolean') context.onMutedChange(message.muted);
        break;
      case 'yt-video-data':
        video = {
...
function mountHls(container: HTMLElement, candidate: Extract<Candidate, { kind: 'hls' }>, context: TransportContext): Transport {
  ...
  video.addEventListener('volumechange', () => { if (!destroyed) context.onMutedChange(video.muted || video.volume === 0); });
