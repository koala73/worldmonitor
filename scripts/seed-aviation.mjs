...
// #6987: the aggregate carries alerts from FAA, international, and NOTAM
// sources. Its health meta must count the population it actually serves,
// not just the FAA sidecar's subset.
const BOOTSTRAP_META_KEY = 'seed-meta:aviation:delays-bootstrap';
...
async function writeDelaysBootstrap(intlOverride) {
  try {
    ...
    if (ok) {
      // #6987: write the aggregate's own meta so /api/health counts what it
      // serves. Without this, a quiet FAA window writes recordCount=0 to
      // seed-meta:aviation:faa and the combined aggregate reads EMPTY_DATA
      // even though it still carries international + NOTAM alerts.
      await upstashSet(
        BOOTSTRAP_META_KEY,
        { fetchedAt: Date.now(), recordCount: payload.alerts.length },
        BOOTSTRAP_TTL,
      );
