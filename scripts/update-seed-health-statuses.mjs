...
export function buildSeedHealthStatuses(rawAcceptance) {
...
function readPreviousObservation({ repository, shas }) {
  for (const sha of shas) {
    ...
    if (newestSeedStatus?.context === ACCEPTANCE_CONTEXT) {
      return { sha, statuses: latestStatusesByContext([statuses], STATUS_PREFIX) };
    }
  }
  return { sha: null, statuses: new Map() };
}
...
function main() {
  ...
  const current = buildSeedHealthStatuses(observation.acceptance);
  ...
  const updates = previousObservation.sha === sha
