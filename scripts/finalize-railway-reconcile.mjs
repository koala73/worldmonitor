...
export function verifierFailureReason(error, manifest) {
  ...
  if (error instanceof ReconcileAuthorizationError
    && ['MAIN_MOVED', 'GATE_NOT_GREEN'].includes(error.code)) {
    return 'STALLED';
  }
...
export function assertManifestProvenance(manifest, expectedHead, env) {
  ...
  if (manifest.intent.producer.repository !== env.GITHUB_REPOSITORY
    ...
    || manifest.intent.producer.runAttempt !== Number(env.GITHUB_RUN_ATTEMPT)) {
...
export async function finalizeRailwayReconcile({
  ...
  authorizeCurrent = readExactCurrentMainAuthorization,
  now = Date.now,
  sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms)),
}) {
  const startedAt = now();
  const manifest = validateResultManifest(uncheckedManifest);
  assertManifestProvenance(manifest, expectedHead, env);
