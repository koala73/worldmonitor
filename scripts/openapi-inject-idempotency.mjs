...
function injectJson(spec) {
  ...
  for (const ops of Object.values(spec.paths ?? {})) {
    ...
    // description + content) — merge in only the replay-marker headers.
    const success = post.responses['200'];
    if (success && typeof success === 'object' && !eq(success.headers, SUCCESS_HEADERS)) {
      success.headers = clone(SUCCESS_HEADERS);
      changed = true;
    }
  }
  return changed;
