...
export function validateSigningConfig({ os, sign, env, log = console.log }) {
  if (!sign) {
    return { signEnabled: false, reason: 'signing disabled', missing: [] };
  }
  ...
  if (os === 'macos') {
    ...
    if (!hasIdentity || !hasProvider) {
      log('[desktop-package] signing is optional; continuing with an unsigned build because no macOS signing credentials were provided.');
      return { signEnabled: false, reason: 'missing macos signing credentials', missing: ['TAURI_BUNDLE_MACOS_SIGNING_IDENTITY or APPLE_SIGNING_IDENTITY', 'TAURI_BUNDLE_MACOS_PROVIDER_SHORT_NAME'] };
    }
  }
  ...
  if (os === 'windows') {
    ...
    if (!hasThumbprint && !hasPfx) {
      log('[desktop-package] signing is optional; continuing with an unsigned build because no Windows signing credentials were provided.');
      return { signEnabled: false, reason: 'missing windows signing credentials', missing: ['TAURI_BUNDLE_WINDOWS_CERTIFICATE_THUMBPRINT or TAURI_BUNDLE_WINDOWS_CERTIFICATE + TAURI_BUNDLE_WINDOWS_CERTIFICATE_PASSWORD'] };
    }
  }

  return { signEnabled: true, reason: 'signing enabled', missing: [] };
}
...
function main() {
  ...
  const signingState = validateSigningConfig({ os, sign, env, log: console.log });
  if (signingState.signEnabled) {
    console.log('[desktop-package] signing enabled for this build.');
  }
  ...
  console.log(`[desktop-package] OS=${os} VARIANT=${variant} BUNDLES=${bundles} SIGN=${signingState.signEnabled ? 'on' : 'off'} (${signingState.reason})`);
