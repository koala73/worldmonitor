...
async function sendDiscord(webhookEnvelope, text) {
  try {
    const url = decrypt(webhookEnvelope);
    const hostname = new URL(url).hostname;
    const addrs = await dns.resolve4(hostname);
...
async function sendWebhook(webhookEnvelope, payload) {
  try {
    const url = decrypt(webhookEnvelope);
...
Current snapshot: ${JSON.stringify({ topRiskCountries: currentLandscape.topRiskCountries, gpsZoneCount: currentLandscape.gpsZoneCount, unrestCount: currentLandscape.unrestCount })}`;
    ...
    for (const ch of deliverable) {
      let ok = false;
      if (ch.channelType === 'telegram' && ch.chatId) ok = await sendTelegram(ch.chatId, text);
      else if (ch.channelType === 'slack' && ch.webhookEnvelope) ok = await sendSlack(ch.webhookEnvelope, text);
      else if (ch.channelType === 'discord' && ch.webhookEnvelope) ok = await sendDiscord(ch.webhookEnvelope, text);
