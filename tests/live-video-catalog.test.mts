...
const newsPanel = readFileSync(new URL('../src/components/LiveNewsPanel.ts', import.meta.url), 'utf8');
function newsChannelIds(arrayName: string): string[] {
  const body = newsPanel.match(new RegExp(`const ${arrayName}[^=]*=[^\\[]*\\[([\\s\\S]*?)\\n\\];`))?.[1] ?? '';
...
describe('live video catalog', () => {
  ...
  it('has exactly one catalog slot per built-in Live News channel', () => {
    assert.ok(builtinNewsIds.size > 50, 'no Live News channel ids found in LiveNewsPanel.ts');
  ...
  it('keeps stream addresses out of LiveNewsPanel.ts', () => {
    assert.doesNotMatch(newsPanel, /\.m3u8/, 'HLS stream URLs belong in src/config/live-video-sources.ts');
    assert.doesNotMatch(newsPanel, /fallbackVideoId:\s*'/, 'video ids belong in src/config/live-video-sources.ts');
  });
