...
for (const [name, call, empty, present, malformed] of [
  ...
  ['advisories', () => listSecurityAdvisories({} as never, {}), { advisories: [], byCountry: {} }, { advisories: [advisory], byCountry: { UA: 'caution' } }, { advisories: [{ ...advisory, pubDate: 'bad' }], byCountry: {} }],
] as const) {
  test(`${name}: empty/hit succeed; miss, malformed and Redis failure are unavailable; repaired seed recovers`, async t => {
    ...
    for (const invalid of [null, {}, malformed, { ...present, fallback: true }, ...(name === 'satellites' ? [{ satellites: [{ ...satellite, line1: 'broken' }] }] : [])]) {
      value = invalid;
...
test('GDELT topics distinguish empty matches from unavailable or malformed seed and read errors', async t => {
  ...
  failure = false;
  value = { topics: [{ id: 'military', articles: [] }] };
  assert.deepEqual((await call()).articles, []);
  assert.equal((await call()).error, '');
  value = { topics: [{ id: 'military', articles: [article] }] };
