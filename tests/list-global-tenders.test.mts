...
import { filterAndPaginateTenders } from '../server/worldmonitor/economic/v1/list-global-tenders';
...
test('filters by country, query, category, money and deadline without treating missing values as matches', () => {
  ...
  assert.deepEqual(result.appliedFilters, ['country', 'status', 'deadline_from', 'deadline_to', 'min_value', 'max_value', 'currency', 'category', 'query']);
});
...
test('uses bounded cursor pagination and stable sorting', () => {
  ...
  const request = { country: '', countries: [], region: '', source: '', status: '', deadlineFrom: '', deadlineTo: '', minValue: 0, maxValue: 0, currency: '', category: '', query: '', pageSize: 2, cursor: '', sort: 'closing_soon' };
