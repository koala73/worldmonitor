...
describe('deploy/cache configuration guardrails', () => {
  ...
  it('caches the self-hosted MapLibre RTL plugin as a public static asset', () => {
    // MapLibre's worker fetch rejects non-2xx, including raw 304. The SPA HTML
    // catch-all used to stamp this vendor script with private no-cache, which
    // forced revalidation and surfaced `Failed to load …/mapbox-gl-rtl-text.min.js: 304`
    // (Sentry WORLDMONITOR-12Y). Keep it out of the HTML catch-all and give it
    // a public TTL like other unhashed static scripts.
    assert.ok(
      SPA_HTML_CACHE_SOURCE.includes('|mapbox-gl-rtl-text\\.min\\.js|'),
      'HTML cache catch-all must exclude the RTL plugin script',
    );
    assert.equal(
      sourceToRegExp(SPA_HTML_CACHE_SOURCE).test('/mapbox-gl-rtl-text.min.js'),
      false,
    );
    assert.equal(
      effectiveCacheControl('/mapbox-gl-rtl-text.min.js'),
