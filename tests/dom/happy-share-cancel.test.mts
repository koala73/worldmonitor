...
it.each([['AbortError', 0], ['NotAllowedError', 1], ['DataError', 1]])('handles %s without mistaking a failure for cancellation', async (name, calls) => {
  ...
  const ctx = new Proxy({}, { get: (_target, key) => key === 'measureText' ? () => ({ width: 10 }) : key === 'createLinearGradient' ? () => ({ addColorStop() {} }) : () => {} });
  ...
  vi.stubGlobal('ClipboardItem', class { constructor(_data: unknown) {} });
  vi.stubGlobal('navigator', { share: vi.fn().mockRejectedValue(new DOMException('share ended', name)), canShare: () => true, clipboard: { write } });
  await shareHappyCard({ title: 'Test news', source: 'test', link: 'https://example.com', pubDate: new Date() });
