...
// ── generateWhyMatters — v12 endpoint-cache cross-read (#4914) ─────────────
//
// The analyst endpoint (api/internal/brief-why-matters.ts) caches its
// envelope at brief:llm:whymatters:v12:{hashBriefStory} — the SAME story
...
describe('generateWhyMatters — v12 endpoint-cache cross-read (#4914)', () => {
  const ENDPOINT_PROSE = 'Closure of the Strait of Hormuz would freeze a fifth of seaborne crude and force allied navies to respond.';

  it('pins the endpoint cache to v12 and its shadow cohort to v7', async () => {
    const { readFile } = await import('node:fs/promises');
    const src = await readFile(new URL('../api/internal/brief-why-matters.ts', import.meta.url), 'utf8');
    assert.match(src, /const cacheKey = `brief:llm:whymatters:v12:\$\{hash\}`;/);
  ...
  async function seedV10(cache, s, envelopeOverrides = {}) {
    ...
    cache.store.set(`brief:llm:whymatters:v12:${hash}`, {
      whyMatters: ENDPOINT_PROSE,
      producedBy: 'analyst',
      ...envelopeOverrides,
    });
    return hash;
  ...
  it('reuses the v12 envelope when the analyst endpoint call fails — no paid LLM call', async () => {
    const cache = makeCache();
    const s = story();
    await seedV10(cache, s);
    const llm = makeLLM(() => { throw new Error('must not pay direct-Gemini when v12 is warm'); });
    const out = await generateWhyMatters(s, {
      ...cache,
      callLLM: llm.callLLM,
      callAnalystWhyMatters: async () => { throw new Error('endpoint down'); },
    });
    assert.equal(out, ENDPOINT_PROSE);
    assert.equal(llm.calls.length, 0, 'v12 hit must short-circuit the legacy chain');
  });

  it('reuses the v12 envelope when no analyst endpoint is configured at all', async () => {
    const cache = makeCache();
    const s = story();
    await seedV10(cache, s);
  ...
  it('malformed v12 envelope falls through to the legacy chain', async () => {
    const cache = makeCache();
    const s = story();
    const hash = await hashBriefStory(s);
    cache.store.set(`brief:llm:whymatters:v12:${hash}`, { whyMatters: 'too short' });
    const llm = makeLLM('Closure of the Strait of Hormuz would spike oil prices globally.');
    const out = await generateWhyMatters(s, { ...cache, callLLM: llm.callLLM });
    assert.equal(out, 'Closure of the Strait of Hormuz would spike oil prices globally.');
    assert.equal(llm.calls.length, 1, 'invalid v12 payload must not be served — legacy chain pays once');
  });

  it('v12 sensitivity-stub prose is rejected, not served', async () => {
    const cache = makeCache();
    const s = story();
  ...
  it('v12 max-token clips are rejected, not served', async () => {
    const cache = makeCache();
    const s = story();
    await seedV10(cache, s, {
  ...
  it('v12 read is read-only — the legacy path must not copy into or overwrite the v12 namespace', async () => {
    const cache = makeCache();
    const s = story();
    const llm = makeLLM('Closure of the Strait of Hormuz would spike oil prices globally.');
    await generateWhyMatters(s, { ...cache, callLLM: llm.callLLM });
    const v12Keys = [...cache.store.keys()].filter((k) => k.startsWith('brief:llm:whymatters:v12:'));
    assert.equal(v12Keys.length, 0, 'legacy fallback output must stay in the v6 namespace');
  });
});
