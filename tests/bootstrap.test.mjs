...
describe('Bootstrap key hydration coverage', () => {
  it('every bootstrap key has a getHydratedData consumer in src/', () => {
    ...
    const PENDING_CONSUMERS = new Set(['chokepointTransits', 'correlationCards', 'euGasStorage', 'chokepointBaselines', 'imfMacro', 'portwatchChokepointsRef', 'portwatchPortActivity', 'energySpineCountries', 'chokepointFlows', 'oilStocksAnalysis', 'lngVulnerability', 'emberElectricityAll']);
    for (const key of keys) {
