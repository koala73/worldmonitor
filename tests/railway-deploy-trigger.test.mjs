...
describe('protected leased mutation orchestration', () => {
  ...
  it('stops later calls when main moves and records a mutation partial', async () => {
    const events = [];
    const plans = [deployPlan('a'), deployPlan('b')];
    ...
    assert.equal(result.result.outcome, 'MUTATION_PARTIAL');
    assert.deepEqual(result.result.entries.map((entry) => entry.outcome), ['TRIGGERED', 'FAILED']);
