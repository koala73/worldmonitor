...
describe('free-account idle-gap allowance', () => {
  function memoryPipeline(store) {
    return async (ops) => {
      const out = [];
      for (const [cmd, key, arg] of ops) {
        if (cmd === 'INCR') {
          ...
          out.push({ result: store.has(key) ? store.get(key) : null });
        } else if (cmd === 'SET') {
          store.set(key, arg);
          out.push({ result: 'OK' });
        } else if (cmd === 'EXPIRE') {
  ...
  it('allows the first call and opens one request window', async () => {
    const store = new Map();
    const now = Date.UTC(2026, 7, 17, 12, 0, 0);
    const result = await reserveFreeAccountAllowance(
      'user_free_1',
      memoryPipeline(store),
      now,
    );
    assert.equal(result.ok, true);
    assert.equal(store.get(freeAccountCallsKey('user_free_1', now)), 1);
    assert.equal(store.get(freeAccountRequestsKey('user_free_1', now)), 1);
    assert.equal(store.get(freeAccountLastActivityKey('user_free_1')), String(now));
  });

  it('same idle window does not burn another request', async () => {
    const store = new Map();
    const now = Date.UTC(2026, 7, 17, 12, 0, 0);
    const pipe = memoryPipeline(store);
    assert.equal((await reserveFreeAccountAllowance('user_free_2', pipe, now)).ok, true);
    assert.equal(
      (await reserveFreeAccountAllowance('user_free_2', pipe, now + 60_000)).ok,
      true,
    );
    assert.equal(store.get(freeAccountRequestsKey('user_free_2', now)), 1);
    assert.equal(store.get(freeAccountCallsKey('user_free_2', now)), 2);
  });

  it('enforces the five-call ceiling', async () => {
    const store = new Map();
    const now = Date.UTC(2026, 7, 17, 12, 0, 0);
    const pipe = memoryPipeline(store);
    for (let i = 0; i < FREE_ACCOUNT_CALLS_PER_DAY; i += 1) {
      assert.equal((await reserveFreeAccountAllowance('user_free_3', pipe, now + i * 1000)).ok, true);
    }
    const sixth = await reserveFreeAccountAllowance('user_free_3', pipe, now + 10_000);
    assert.equal(sixth.ok, false);
    assert.equal(sixth.reason, 'allowance-exhausted');
  });

  it('enforces the three request-window ceiling across idle gaps', async () => {
    const store = new Map();
    const now = Date.UTC(2026, 7, 17, 12, 0, 0);
    const pipe = memoryPipeline(store);
    for (let i = 0; i < FREE_ACCOUNT_REQUESTS_PER_DAY; i += 1) {
      const t = now + i * (FREE_ACCOUNT_IDLE_GAP_MS + 1);
      assert.equal((await reserveFreeAccountAllowance('user_free_4', pipe, t)).ok, true);
    }
    const fourthWindow = await reserveFreeAccountAllowance(
      'user_free_4',
      pipe,
      now + FREE_ACCOUNT_REQUESTS_PER_DAY * (FREE_ACCOUNT_IDLE_GAP_MS + 1),
    );
    assert.equal(fourthWindow.ok, false);
    assert.equal(fourthWindow.reason, 'allowance-exhausted');
  });

  it('fails closed when Redis is unavailable', async () => {
    const result = await reserveFreeAccountAllowance('user_free_5', async () => {
