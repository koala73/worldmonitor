...
describe('resilience handlers', () => {
  it('computes and caches a country score with domains, trend metadata, and history writes', async () => {
    ...
    sortedSets.set('resilience:history:v3:US', [
    ...
    const history = sortedSets.get('resilience:history:v3:US') ?? [];
    ...
    assert.equal((sortedSets.get('resilience:history:v3:US') ?? []).length, history.length, 'cache hit must not append history');
  });
});
