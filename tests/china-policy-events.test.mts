...
describe('China official policy adapters (#5576)', () => {
  ...
  // against SCALING_POSITIVE_CONTROL_RATIO (11x): a linear scan stays ~4x
  // (GC noise ~10x), while a real quadratic's median stays in the ~14–16x
  // band — and still clears 11x when CI compresses it into the 11–12 gap.
  ...
  // Positive-control floor sits 1x under the guard ceiling. Under 16-way CI
  // load the median attempt ratio has compressed into the 11–12 gap (observed
  // 11.8x on #8019; same timing class as #7910 / #7238) while linear (~4x) and
  // linear+GC (~10x) stay below. Keep the guard itself at SCALING_CEILING_RATIO
  // so production scans are not given more slack.
  const SCALING_POSITIVE_CONTROL_RATIO = SCALING_CEILING_RATIO - 1;

  // Guard ceiling definition. The positive control tracks it via
  // SCALING_POSITIVE_CONTROL_RATIO so a retune of the guard still moves the
  // control's floor, without forcing them to share one brittle equality.
  ...
  it('keeps its teeth: a genuinely quadratic scan still trips the gate', () => {
    ...
    // file's runtime.
    //
    // Assert on median per-attempt ratio (11x positive-control floor), not
    // ratio-of-mins: min/min is what keeps the guard stable, but it is the
    // wrong aggregator for proving the gate still fails.
    ...
    assert.ok(
      scaling.medianRatio > SCALING_POSITIVE_CONTROL_RATIO,
