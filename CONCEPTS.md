...
A term — an ordinary word, a vulnerability identifier, or a threat-group designator — appearing materially more often in a recent window than its own recent history predicts. A spike is deliberately harder to earn than "appeared a lot": the term must clear a floor of distinct mentions, exceed its baseline rate by a strict multiplier, and be carried by more than one outlet, so a single prolific source cannot manufacture one. The corroboration count is over Publisher Families, not feed labels — one newsroom shipping five desk feeds is one outlet, and counting the labels instead is what lets a single publisher raise a "several different sources" alert by itself. The multi-outlet test is decided synchronously while spikes are being selected, so a term that fails it is never emitted at all rather than emitted and filtered later. When no baseline exists yet the decision falls back to the floor alone, which is why a corpus that yields no usable baseline silently converts spike detection into simple frequency counting. See also: Sampled Span, Publisher Family.

### Publisher Family

The newsroom behind a feed label, as distinct from the label itself. One publisher ships many feeds — regional desks, section feeds, syndication partners — and every corroboration claim ("mentioned across N different sources") is a statement about newsrooms, so anything counting distinct sources must fold labels into families first. Unmapped labels each become their own singleton family, so an unrecognized feed is treated as an independent publisher rather than silently merged; the family id is case-normalized, because a label whose casing drifts between two configs would otherwise resolve to two families and manufacture a second publisher out of one feed. The family is a counting key, not display text — the curated publisher name is what a reader sees. See also: Keyword Spike.
