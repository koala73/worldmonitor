...
import { CHROME_UA, yahooGate } from '../../../_shared/constants.ts';
import { UPSTREAM_TIMEOUT_MS } from './_shared.ts';
...
async function fetchYahooHistory(symbol: string): Promise<{ candles: Candle[]; currency: string } | null> {
  await yahooGate();
...
function getFallbackOverlay(name: string, technical: TechnicalSnapshot, headlines: StockAnalysisHeadline[]): AiOverlay {
...
async function fetchRecentHeadlines(symbol: string, name: string): Promise<StockAnalysisHeadline[]> {
  const query = `${name || symbol} ${symbol} stock when:7d`;
  ...
  try {
    ...
    const parsed = NEWS_XML.parse(xml) as {
...
export async function analyzeStock(
  ...
  const cached = await cachedFetchJson<AnalyzeStockResponse>(cacheKey, CACHE_TTL_SECONDS, async () => {
    ...
    const headlines = includeNews ? await fetchRecentHeadlines(symbol, name) : [];
