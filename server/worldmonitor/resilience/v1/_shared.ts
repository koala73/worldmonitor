...
// v22→v23 bump for PR #4088: importConcentration now derates certainty
// coverage when Comtrade source years are stale, missing, or malformed. The
// raw HHI score is unchanged, but coverage participates in aggregate score
// weighting, so same-formula v22 score payloads must not be served.
...
// PR #4088 import-HHI certainty coverage derating; otherwise pre-derate
// and post-derate aggregate scores can mix inside the rolling trend window.
export const RESILIENCE_HISTORY_KEY_PREFIX = 'resilience:history:v18:';
...
// v22→v23 bump in lockstep with RESILIENCE_SCORE_CACHE_PREFIX for PR #4088
// so the public ranking recomputes against import-HHI certainty-derated score
// entries instead of serving pre-derate aggregates for 12h.
...
// v6→v7: PR #4088 can change aggregate scores through import-HHI certainty
// coverage, so old sensitivity bands must not be served with v23 scores.
