...
// v22→v23 bump for P3-8: infrastructure now treats an observed outage feed
// with zero outages as observed-quiet (score 100), matching cyberDigital. Same
// `pc` formula tag, but infrastructure/live-shock/overall scores move.
...
// P3-8 outage semantics; otherwise pre-change and post-change scores would mix
// inside the rolling 30-day trend window and manufacture deploy-day trends.
...
// v22→v23 bump in lockstep with RESILIENCE_SCORE_CACHE_PREFIX for P3-8 so the
// public ranking recomputes against v23 score entries instead of serving
// pre-fix aggregates for 12h.
...
// v6→v7: P3-8 outage semantics change same-tag `pc` infrastructure/live-shock
// score baselines, so old rank-stability bands must not be served with v23
// scores.
