...
export const VISUAL_INTENT_RE =
  /\b(chart\s+(prices?|data|rates?|trends?|performance|comparison|history)|graph\s+(prices?|data|trends?)|plot\s+(prices?|data|trends?)|visuali[sz]e|create\s+a\s+(chart|graph|dashboard|visualization)|show\s+(me\s+)?(a\s+)?(chart|graph|plot|dashboard|trend)|price\s+(history|over\s+time|comparison|trend|chart)|compare\s+(prices?|rates?|data|performance)|dashboard|candlestick)\b/i;
