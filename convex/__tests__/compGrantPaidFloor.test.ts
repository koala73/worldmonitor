...
test("a Pro goodwill grant preserves paid Business and remains Pro after paid expiry", async () => {
  vi.useFakeTimers();
  ...
  const subscriptionId = await t.run((ctx) => ctx.db.insert("subscriptions", {
    userId: "synthetic_paid_comp", planKey: "api_business", status: "active",
    dodoSubscriptionId: "sub_synthetic_paid_comp", dodoProductId: PRODUCT_CATALOG.api_business.dodoProductId!,
  ...
  expect(await read()).toMatchObject({ planKey: "api_business", validUntil: now + 30 * day });
