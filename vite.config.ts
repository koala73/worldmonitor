...
function sebufApiPlugin(): Plugin {
  ...
  async function buildRouter() {
    const [
      ...
      reitsServerMod, reitsHandlerMod,
    ] = await Promise.all([
        import('./server/router'),
        import('./server/cors'),
        import('./server/error-mapper'),
        import('./src/generated/server/worldmonitor/reits/v1/service_server'),
        import('./server/worldmonitor/reits/v1/handler'),
      ]);

    const serverOptions = { onError: errorMod.mapErrorToResponse };
    const allRoutes = [
      ...reitsServerMod.createReitsServiceRoutes(reitsHandlerMod.reitsHandler, serverOptions),
    ];
