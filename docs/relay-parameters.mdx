...
| `AIS_RECONNECT_BASE_MS` | Railway | `5000` (min `1`) | No | Base delay of the exponential upstream reconnect ladder. |
| `AIS_RECONNECT_MAX_MS` | Railway | `300000` (min `1`) | No | Ordinary ceiling for the reconnect ladder. |
| `AIS_THROTTLE_RECONNECT_MAX_MS` | Railway | `1800000` (min `1`) | No | Escalated ceiling applied once the provider throttles persistently. |
