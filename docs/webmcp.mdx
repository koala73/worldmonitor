...
| Discover map-layer IDs, including disabled layers | `list_map_layers` | Page the catalog. Use `nextCursor` when present, and only with the same `monitor`, `renderer`, and `state`. Inspect `available` and `reason` before enabling; those fields are current-page `set_map_layers` gates, not the requested monitor. This tool is read-only and does not load map datasets. |
