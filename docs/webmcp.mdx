...
On a browser that omits the target-side invocation signal, the reversible view-state tools still run: because such a browser cannot cancel them, an invocation you abort still completes and its visible dashboard change still lands. `openCountryBrief` and `set_map_layers` reject as unavailable there instead. `open_search_result` still runs a view-state result (for example opening an already-enabled live-webcam panel) and rejects a persistent, quota-consuming, or external-navigation result, including a panel tab deep-link that writes settings. The country tool can consume daily LLM allowance after caller cancellation, while layer writes persist selection beyond the session. The read-only `list_map_layers` catalog still runs. See [Availability](#availability).
