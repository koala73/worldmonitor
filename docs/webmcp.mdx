...
| `set_panel_enabled` | Required string `panelId`, 1–96 characters, pattern `^[a-z0-9][a-z0-9@_-]*$`; required boolean `enabled`; no additional properties. | Enables or disables a catalog panel through the same settings persist/apply path a person uses. Returns the requested state, effective state, and whether anything changed. Unknown, incompatible, unentitled, and free-tier-capped panels are denied. Requires target-side cancellation. |
