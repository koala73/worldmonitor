...
**ChatGPT does not discover WebMCP tools merely because a prompt names a website.** A normal ChatGPT conversation, including one in the iOS or Android app, is not the Chrome tab that owns `document.modelContext`. To use WorldMonitor from ChatGPT, first add `https://worldmonitor.app/mcp` as a custom app in ChatGPT's developer mode, then enable that app in a new conversation. See [ChatGPT setup](/mcp-overview#chatgpt).
</Warning>
