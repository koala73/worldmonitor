...
`openCountryBrief`, `set_map_layers`, and `open_search_result` stay gated and reject as unavailable. `openCountryBrief` can start server-side LLM generation (`premiumFetch` → `/api/intelligence/v1/get-country-intel-brief` → `callLlm`) that consumes the caller's daily allowance even if the browser later cancels the caller-side promise. The two layer tools persist the map-layer selection to browser storage, and enabling the AIS layer also opens a network stream. These effects can outlive cancellation, so the tools become available only on a browser that supplies the target-side signal.
...
| Discover map-layer IDs, including disabled layers | `list_map_layers` | Page the catalog. Use `nextCursor` when present. Inspect `available` and `reason` before enabling. This tool is read-only and does not load map datasets. |
