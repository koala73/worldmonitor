...
WorldMonitor still runs its dashboard-changing tools there. Each one only moves visible dashboard view state on the person's own screen — no navigation, no writes, no external side effects — so the worst case of an uncancellable invocation is a visible change the person can undo. Refusing to run them instead would disable every dashboard-changing tool on every browser released so far, leaving only the read-only pair, in exchange for preventing a map move. A tool that could ever do something a person cannot simply look at and undo stays gated and rejects as unavailable until the browser supplies the target-side signal.
...
All eight run on a browser that omits the target-side invocation signal. Because such a browser cannot cancel them, an invocation you abort still completes and its visible dashboard change still lands — see [Availability](#availability). Tools whose effect a person could not simply undo are gated instead and reject as unavailable; none of the eight are gated today.
