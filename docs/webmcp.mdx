...
4. Start WorldMonitor locally. Open `/dashboard` for the twenty-tool dashboard, not `/embed`. To inspect the static two-tool homepage, first run `npm run build:pro`, then open `/pro/welcome.html`. The local Vite route `/` loads the dashboard SPA; only production rewrites `/` to the welcome page.
...
`openCountryBrief`, `switch_monitor`, `set_map_layers`, `select_dashboard_tab`, `create_dashboard_tab`, `rename_dashboard_tab`, and `delete_dashboard_tab` stay gated and reject as unavailable. `openCountryBrief` can start server-side LLM generation (`premiumFetch` → `/api/intelligence/v1/get-country-intel-brief` → `callLlm`) that consumes the caller's daily allowance even if the browser later cancels the caller-side promise. `switch_monitor` can persist a desktop variant and reload or navigate the current tab to another origin. `set_map_layers` persists the map-layer selection to browser storage, and enabling the AIS layer also opens a network stream. Dashboard tab mutations persist `worldmonitor-tabs-v1` plus the live panel workspace. `open_search_result` is a selector: it runs a view-state result such as opening an already-enabled panel with no tab deep-link, and still rejects persistent, quota-consuming, or external-navigation results. Tab deep-links such as `panel:consumer-prices@world` persist panel settings, so they stay blocked without a target-side signal. These latter effects can outlive cancellation, so they become available only on a browser that supplies the target-side signal. The read-only `list_map_layers` and `list_dashboard_tabs` catalogs stay available.
...
Every dashboard variant registers the same twenty imperative tools. Registration is stable across sign-in and entitlement changes; each invocation rechecks the live state.

On a browser that omits the target-side invocation signal, the reversible view-state tools still run: because such a browser cannot cancel them, an invocation you abort still completes and its visible dashboard change still lands. `openCountryBrief`, `switch_monitor`, `set_map_layers`, `select_dashboard_tab`, `create_dashboard_tab`, `rename_dashboard_tab`, and `delete_dashboard_tab` reject as unavailable there instead. `open_search_result` still runs a view-state result (for example opening an already-enabled live-webcam panel) and rejects a persistent, quota-consuming, or external-navigation result, including a panel tab deep-link that writes settings. The country tool can consume daily LLM allowance after caller cancellation, monitor switching can persist and reload or navigate to another origin, and layer writes and tab mutations persist beyond the session. The read-only `list_map_layers` and `list_dashboard_tabs` catalogs still run. See [Availability](#availability).
...
| `open_dashboard_panel` | Required string `panelId`, 1–96 characters, pattern `^[A-Za-z0-9][A-Za-z0-9@_-]*$`; no additional properties. | Opens and scrolls to an available, currently enabled panel through the entitlement-aware UI path. Disabled panels return `panel_disabled`; a person can enable them from dashboard search or settings. This tool does not enable panels itself. |
...
| A call returns `target_cancellation_unsupported` | The browser accepted WebMCP but did not give the page the invocation's `AbortSignal` | Use read-only or reversible view-state tools. Do not bypass the denial for `openCountryBrief`, `switch_monitor`, `set_map_layers`, or dashboard tab mutations. For `open_search_result`, choose a view-state result or wait for a host that can cancel persistent work. |
...
3. Classify the tool as `read-only`, `view-state`, `cancellation-required`, or `result-dependent`. A new imperative tool must have an explicit cancellation policy before TypeScript can accept the inventory.
...
| `src/services/webmcp-map-layer-catalog.ts` | Paginated map-layer catalog derivation, filters, and availability reasons aligned with `set_map_layers`. |
| `src/services/webmcp-panel-catalog.ts` | Canonical panel catalog pagination, filters, entitlement overlay, and stable unavailable reasons. |
| `src/App.ts` and `src/app/webmcp-dashboard.ts` | Startup ordering, UI-readiness gate, teardown, dashboard context, catalog snapshots, and human-path action binding. |
...
| `src/app/webmcp-search-controller.ts`, `src/app/webmcp-search-effects.ts`, and `src/app/search-selection-dispatcher.ts` | Opaque search capabilities, bound result effect classes, invalidation, live-state revalidation, and visible result selection. |
| `src/app/panel-layout.ts`, `src/services/tab-store.ts`, and `src/services/dashboard-tab-actions.ts` | Dashboard-tab persistence, name and ID limits, and WebMCP list/select/create/rename/delete actions. |
...
For a browser-facing contract change, also run `npm run test:dom` and `npm run test:e2e:webmcp`. For a release, complete the production same-SHA smoke above and preserve its JSON evidence files. A missing browser, origin-trial token, credential, or deployed SHA is a named verification gate; it is not permission to weaken the suite.
