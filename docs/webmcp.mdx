...
4. Start WorldMonitor locally. Open `/dashboard` for the dashboard SPA tool inventory, not `/embed`. To inspect the static two-tool homepage, first run `npm run build:pro`, then open `/pro/welcome.html`. The local Vite route `/` loads the dashboard SPA; only production rewrites `/` to the welcome page.
...
Every dashboard variant registers the same imperative SPA inventory. Registration is stable across sign-in and entitlement changes; each invocation rechecks the live state.
...
| `src/App.ts` and `src/app/webmcp-dashboard.ts` and `src/app/event-handlers.ts` | Startup ordering, UI-readiness gate, teardown, dashboard context, monitor/settings/alerts navigation, and human-path action binding. |
