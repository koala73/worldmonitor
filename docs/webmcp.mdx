...
4. Start WorldMonitor locally. Open `/dashboard` for the thirteen-tool dashboard, not `/embed`. To inspect the static two-tool homepage, first run `npm run build:pro`, then open `/pro/welcome.html`. The local Vite route `/` loads the dashboard SPA; only production rewrites `/` to the welcome page.
...
`openCountryBrief`, `set_map_layers`, `open_search_result`, `select_dashboard_tab`, `create_dashboard_tab`, `rename_dashboard_tab`, and `delete_dashboard_tab` stay gated and reject as unavailable. `openCountryBrief` can start server-side LLM generation (`premiumFetch` → `/api/intelligence/v1/get-country-intel-brief` → `callLlm`) that consumes the caller's daily allowance even if the browser later cancels the caller-side promise. The two layer tools persist the map-layer selection to browser storage, and enabling the AIS layer also opens a network stream. Dashboard tab mutations persist `worldmonitor-tabs-v1` plus the live panel workspace. These effects can outlive cancellation, so the tools become available only on a browser that supplies the target-side signal.
...
Every dashboard variant registers the same thirteen imperative tools. Registration is stable across sign-in and entitlement changes; each invocation rechecks the live state.

On a browser that omits the target-side invocation signal, the reversible view-state tools still run: because such a browser cannot cancel them, an invocation you abort still completes and its visible dashboard change still lands. `openCountryBrief`, `set_map_layers`, `open_search_result`, `select_dashboard_tab`, `create_dashboard_tab`, `rename_dashboard_tab`, and `delete_dashboard_tab` reject as unavailable there instead. The country tool can consume daily LLM allowance after caller cancellation, while the layer tools persist selection beyond the session and tab mutations persist named workspaces. See [Availability](#availability).
...
| `rename_dashboard_tab` | Required string `tabId` plus required string `name`, 1–40 characters; no additional properties. | Renames a tab by stable id. Names match the dashboard tab bar. Renaming to the current name is a successful no-op. |
| `delete_dashboard_tab` | Required string `tabId` plus required boolean `confirm`; no additional properties. | Deletes a tab after `confirm` is exactly `true`. The last remaining tab cannot be deleted (`last_tab`). |
...
| A call returns `target_cancellation_unsupported` | The browser accepted WebMCP but did not give the page the invocation's `AbortSignal` | Use read-only or reversible view-state tools. Do not bypass the denial for `openCountryBrief`, `set_map_layers`, `open_search_result`, or dashboard tab mutations. |
...
| A tab mutation returns `persist_failed` with `persisted: false` | The visible tab changed this session, but `worldmonitor-tabs-v1` could not be written (private mode or quota) | Do not treat the result as durable. A reload will restore the previous tabs. Ask the person to free storage or leave private mode, then list tabs again. |
...
| `src/app/panel-layout.ts`, `src/services/tab-store.ts`, and `src/services/dashboard-tab-actions.ts` | Dashboard tab persistence, name and ID limits, and WebMCP list/select/create/rename/delete. |
