...
4. Start WorldMonitor locally. Open `/dashboard` for the fourteen-tool dashboard, not `/embed`. To inspect the static two-tool homepage, first run `npm run build:pro`, then open `/pro/welcome.html`. The local Vite route `/` loads the dashboard SPA; only production rewrites `/` to the welcome page.
...
Every dashboard variant registers the same fourteen imperative tools. Registration is stable across sign-in and entitlement changes; each invocation rechecks the live state.
