...
4. Start WorldMonitor locally. Open `/dashboard` for the twenty-one-tool dashboard, not `/embed`. To inspect the static two-tool homepage, first run `npm run build:pro`, then open `/pro/welcome.html`. The local Vite route `/` loads the dashboard SPA; only production rewrites `/` to the welcome page.
...
Every dashboard variant registers the same twenty-one imperative tools. Registration is stable across sign-in and entitlement changes; each invocation rechecks the live state.

On a browser that omits the target-side invocation signal, the reversible view-state tools still run: because such a browser cannot cancel them, an invocation you abort still completes and its visible dashboard change still lands. `openCountryBrief`, `switch_monitor`, `set_panel_enabled`, `set_map_layers`, `select_dashboard_tab`, `create_dashboard_tab`, `rename_dashboard_tab`, and `delete_dashboard_tab` reject as unavailable there instead. `open_search_result` still runs a view-state result (for example opening an already-enabled live-webcam panel) and rejects a persistent, quota-consuming, or external-navigation result, including a panel tab deep-link that writes settings. The country tool can consume daily LLM allowance after caller cancellation, monitor switching can persist and reload or navigate to another origin, and panel, layer, and tab writes persist beyond the session. The read-only `list_map_layers` and `list_dashboard_tabs` catalogs still run. See [Availability](#availability).
