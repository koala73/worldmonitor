...
`openCountryBrief`, `set_map_layers`, `select_dashboard_tab`, `create_dashboard_tab`, `rename_dashboard_tab`, and `delete_dashboard_tab` stay gated and reject as unavailable. `openCountryBrief` can start server-side LLM generation (`premiumFetch` → `/api/intelligence/v1/get-country-intel-brief` → `callLlm`) that consumes the caller's daily allowance even if the browser later cancels the caller-side promise. `set_map_layers` persists the map-layer selection to browser storage, and enabling the AIS layer also opens a network stream. Dashboard tab mutations persist `worldmonitor-tabs-v1` plus the live panel workspace. `open_search_result` is a selector: it runs a view-state result such as opening an already-enabled panel with no tab deep-link, and still rejects persistent, quota-consuming, or external-navigation results. Tab deep-links such as `panel:consumer-prices@world` persist panel settings, so they stay blocked without a target-side signal. These latter effects can outlive cancellation, so they become available only on a browser that supplies the target-side signal. The read-only `list_dashboard_tabs` tool stays available.
...
On a browser that omits the target-side invocation signal, the reversible view-state tools still run: because such a browser cannot cancel them, an invocation you abort still completes and its visible dashboard change still lands. `openCountryBrief`, `set_map_layers`, `select_dashboard_tab`, `create_dashboard_tab`, `rename_dashboard_tab`, and `delete_dashboard_tab` reject as unavailable there instead. `open_search_result` still runs a view-state result (for example opening an already-enabled live-webcam panel) and rejects a persistent, quota-consuming, or external-navigation result, including a panel tab deep-link that writes settings. The country tool can consume daily LLM allowance after caller cancellation, while layer writes and tab mutations persist beyond the session. The read-only `list_dashboard_tabs` tool still runs. See [Availability](#availability).
...
|---|---|---|
| Read active workspace panel/map state | `get_dashboard_context` | Read the returned variant, map view, layers, and panel IDs. Honor the `*Truncated` fields instead of assuming a shortened list is complete. |
...
Do not guess panel IDs, layer IDs, tab IDs, result keys, entitlements, or hidden data. Read the current page state, invoke one bounded action, inspect its result and visible effect, then continue.
...
| A call returns `target_cancellation_unsupported` | The browser accepted WebMCP but did not give the page the invocation's `AbortSignal` | Use read-only or reversible view-state tools. Do not bypass the denial for `openCountryBrief`, `set_map_layers`, or dashboard tab mutations. For `open_search_result`, choose a view-state result or wait for a host that can cancel persistent work. |
