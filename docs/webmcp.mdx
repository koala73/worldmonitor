...
WorldMonitor still runs the dashboard-changing tools whose effects are either already accounted for by an existing metering or consent system, or land as a visible change a person can overwrite — the gate exists to stop an uncancellable invocation from persisting silently outside any such system, not to stop every tool that carries a cost or a visible effect. `openCountryBrief`, `openSearch`, `open_dashboard_panel`, and `set_map_view` work normally. `openCountryBrief` triggers a server-side LLM brief generation (`premiumFetch` → `/api/intelligence/v1/get-country-intel-brief` → `callLlm`), but that spend is metered per user by the entitlement layer, which counts it against the caller's own daily allowance (500/day paid, 50/day unverified) and refunds it on cap-exceeded — an aborted `openCountryBrief` still consumes one brief generation from the caller's own allowance. `set_map_view` writes the address bar through `history.replaceState`, producing the same share URL the page's link-copy control produces and that the dashboard restores on reload; that write is visible in the address bar and is overwritten by the next human map move.

`set_map_layers` and `open_search_result` stay gated and reject as unavailable, because both persist the map-layer selection to browser storage (and enabling the AIS layer opens a network stream). An uncancellable invocation of either would outlive the session rather than sit on screen waiting to be undone, and moving the map back does not restore the stored value. They become available on a browser that supplies the target-side signal.
...
On a browser that omits the target-side invocation signal, the view-state tools still run: because such a browser cannot cancel them, an invocation you abort still completes and its visible dashboard change still lands. `set_map_layers` and `open_search_result` reject as unavailable there instead, because they persist the layer selection beyond the session. See [Availability](#availability).
