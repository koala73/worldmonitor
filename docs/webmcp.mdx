...
4. Start WorldMonitor locally. Open `/dashboard` for the twenty-eight-tool dashboard, not `/embed`. To inspect the static two-tool homepage, first run `npm run build:pro`, then open `/pro/welcome.html`. The local Vite route `/` loads the dashboard SPA; only production rewrites `/` to the welcome page.
...
3. Select **Site tools** in the browser address bar, then select **Available site tools**. The homepage lists two imperative tools. The dashboard lists twenty-eight.
...
|---|---|---|
| `read-only` | `get_dashboard_context`, `get_access_context`, `list_map_layers`, `list_dashboard_panels`, `search_dashboard`, `list_dashboard_tabs`, `get_panel_layout` | Runs normally. |
| `view-state` | `openSearch`, `open_settings`, `open_alerts`, `open_sign_in`, `open_dashboard_panel`, `set_map_view`, `set_time_range`, `focus_country`, `set_panel_fullscreen` | Runs normally, but caller cancellation cannot stop a visible change that is already in progress. |
| `cancellation-required` | `openCountryBrief`, `switch_monitor`, `set_panel_enabled`, `set_panel_collapsed`, `move_panel`, `set_map_layers`, `set_map_mode`, `select_dashboard_tab`, `create_dashboard_tab`, `rename_dashboard_tab`, `delete_dashboard_tab` | Returns `target_cancellation_unsupported` before starting the action. |
...
Every dashboard variant registers the same twenty-eight imperative tools. Registration is stable across sign-in and entitlement changes. Each invocation rechecks live state and the [host's cancellation support](#host-support-and-cancellation).
