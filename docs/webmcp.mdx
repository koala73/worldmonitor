...
4. Start WorldMonitor locally. Open `/dashboard` for the fifteen-tool dashboard, not `/embed`. To inspect the static two-tool homepage, first run `npm run build:pro`, then open `/pro/welcome.html`. The local Vite route `/` loads the dashboard SPA; only production rewrites `/` to the welcome page.
...
`openCountryBrief`, `switch_monitor`, and `set_map_layers` stay gated and reject as unavailable. `openCountryBrief` can start server-side LLM generation (`premiumFetch` → `/api/intelligence/v1/get-country-intel-brief` → `callLlm`) that consumes the caller's daily allowance even if the browser later cancels the caller-side promise. `switch_monitor` can persist a desktop variant and reload or navigate the current tab to another origin. `set_map_layers` persists the map-layer selection to browser storage, and enabling the AIS layer also opens a network stream. `open_search_result` is a selector: it runs a view-state result such as opening an already-enabled panel with no tab deep-link, and still rejects persistent, quota-consuming, or external-navigation results. Tab deep-links such as `panel:consumer-prices@world` persist panel settings, so they stay blocked without a target-side signal. These latter effects can outlive cancellation, so they become available only on a browser that supplies the target-side signal. The read-only `list_map_layers` catalog stays available.
...
Every dashboard variant registers the same fifteen imperative tools. Registration is stable across sign-in and entitlement changes; each invocation rechecks the live state.

On a browser that omits the target-side invocation signal, the reversible view-state tools still run: because such a browser cannot cancel them, an invocation you abort still completes and its visible dashboard change still lands. `openCountryBrief`, `switch_monitor`, and `set_map_layers` reject as unavailable there instead. `open_search_result` still runs a view-state result (for example opening an already-enabled live-webcam panel) and rejects a persistent, quota-consuming, or external-navigation result, including a panel tab deep-link that writes settings. The country tool can consume daily LLM allowance after caller cancellation, monitor switching can persist and reload or navigate to another origin, and layer writes persist selection beyond the session. The read-only `list_map_layers` catalog still runs. See [Availability](#availability).
...
Do not guess panel IDs, layer IDs, result keys, entitlements, or hidden data. Read the current page state or `list_dashboard_panels`, invoke one bounded action, inspect its result and visible effect, then continue.
...
| A call returns `target_cancellation_unsupported` | The browser accepted WebMCP but did not give the page the invocation's `AbortSignal` | Use read-only or reversible view-state tools. Do not bypass the denial for `openCountryBrief`, `switch_monitor`, or `set_map_layers`. For `open_search_result`, choose a view-state result or wait for a host that can cancel persistent work. |
