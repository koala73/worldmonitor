...
`openCountryBrief` and `set_map_layers` stay gated and reject as unavailable. `openCountryBrief` can start server-side LLM generation (`premiumFetch` → `/api/intelligence/v1/get-country-intel-brief` → `callLlm`) that consumes the caller's daily allowance even if the browser later cancels the caller-side promise. `set_map_layers` persists the map-layer selection to browser storage, and enabling the AIS layer also opens a network stream. `open_search_result` is a selector: it runs a view-state result such as opening an already-enabled panel, and still rejects persistent, quota-consuming, or external-navigation results. These latter effects can outlive cancellation, so they become available only on a browser that supplies the target-side signal.
