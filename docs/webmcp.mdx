...
Chrome 149–151 exposes `registerTool()` but invokes the registered callback with the input alone, not the documented `execute(input, { signal })` shape. On those builds cancellation is inert in both directions: `executeTool()` ignores the `signal` in its options argument, so aborting neither rejects the caller's promise nor stops work already running in the page.

WorldMonitor still runs the dashboard-changing tools whose only effect is visible view state on the person's own screen — no navigation, no writes, no external side effects — because the worst case of an uncancellable invocation there is a visible change the person can undo. `openCountryBrief`, `openSearch`, `open_dashboard_panel`, and `set_map_view` work normally.
...
This cooperative target-side cancellation proof requires the browser to pass the invocation signal to the registered callback. On a one-argument callback implementation — every Chrome released so far — the snippet above never reaches its `AbortError` branch: the abort is ignored, the caller's promise resolves with the tool's normal result, and the work completes inside the page. Treat cancellation as unavailable rather than best-effort on those builds.
