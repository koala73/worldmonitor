...
`openCountryBrief`, `set_map_layers`, and `open_search_result` stay gated and reject as unavailable. `openCountryBrief` can start server-side LLM generation (`premiumFetch` → `/api/intelligence/v1/get-country-intel-brief` → `callLlm`) that consumes the caller's daily allowance even if the browser later cancels the caller-side promise. `set_map_layers` and `open_search_result` persist the map-layer selection to browser storage, and enabling the AIS layer also opens a network stream. These effects can outlive cancellation, so the tools become available only on a browser that supplies the target-side signal. The read-only `list_map_layers` catalog stays available.
...
| `list_map_layers` | Optional `monitor`: `world`, `tech`, `finance`, `commodity`, `energy`, or `happy`. Optional `renderer`: `2d` or `3d`. Optional `state`: `enabled` or `available`. Optional `cursor` matching `^[a-z][A-Za-z0-9_-]*$`, 1–30 characters. Optional integer `limit`: 1–8 (default 6). No additional properties. | Pages the canonical catalog of registered map layers, including disabled ones. Omitting `monitor` lists every non-sunset registered layer; `monitor=world` lists only the world variant. Reuse `cursor` only with the same `monitor`, `renderer`, and `state`; changing filters continues after that ID in the new sequence. Each row has the stable ID, label, enabled state, monitor availability, renderer compatibility, entitlement, and a machine-readable reason when the current page cannot enable the layer with `set_map_layers`. `available` and `reason` follow current-page `set_map_layers` gates, including `target_cancellation_unsupported` when the host omits a target-side AbortSignal, not the requested monitor. Top-level `variant` and `renderer` describe the live page. Does not load map datasets. |
...
| Discover map-layer IDs, including disabled layers | `list_map_layers` | Page the catalog. Use `nextCursor` when present, and only with the same `monitor`, `renderer`, and `state`. Inspect `available` and `reason` before enabling; those fields are current-page `set_map_layers` gates, including host cancellation support, not the requested monitor. This tool is read-only and does not load map datasets. |
