...
| `list_dashboard_tabs` | Empty object; no additional properties. | Read-only snapshot of dashboard tabs as named persistent panel workspaces: stable ids, names, active flag, create availability, and cap lock reason. When `tabsTruncated` is true, `tabCount` is the total persisted workspace count and the tabs array may omit non-active entries. |
...
| List dashboard workspaces | `list_dashboard_tabs` | Use returned tab ids, not display names. Honor `tabsTruncated`. |
