...
|---|---|---|
| `malformed_arguments` | `get_panel_layout`, `set_panel_collapsed`, `move_panel`, `set_panel_fullscreen`, `apply_mission_preset`, `open_mission_picker` | An unknown property, a non-string cursor, a panel ID outside `^[A-Za-z0-9][A-Za-z0-9@_-]*$`, or a preset ID outside `^[a-z][a-z0-9-]*$`. Fix the arguments instead of retrying the same payload. |
...
| `persist_failed` | `set_panel_collapsed`, `move_panel` | The visible layout changed this session, but `worldmonitor-panel-collapsed` (collapse) or `panel-order` and `panel-order-bottom-set` (move) could not be written. The result carries `persisted: false`; do not treat the change as durable. |
...
| `target_cancellation_unsupported` | `set_panel_collapsed`, `move_panel`, `apply_mission_preset` | The host did not give the page the invocation's `AbortSignal`. Nothing was written. See [Host support and cancellation](#host-support-and-cancellation). |

Two failures arrive as rejected promises rather than bounded results. A destroyed dashboard rejects every panel-layout and mission-preset tool with a `WebMcpToolError` naming `app_destroyed` as its reason, and `list_mission_presets` rejects malformed arguments and an unrecognized variant instead of returning them. Handle both on the rejection path.
...
| A tab or layout mutation returns `persist_failed` with `persisted: false` | The visible change landed this session, but its storage key could not be written — `worldmonitor-tabs-v1` for tabs, `worldmonitor-panel-collapsed` for collapse, `panel-order` and `panel-order-bottom-set` for a move | Do not treat the result as durable. Ask the person to free storage or leave private mode, then read the current state again. |
