...
**Trusting an existing code comment.** `pro-test/src/sentry-filter-policy.ts` argued the dashboard bundle "mints its own `signal timed out` DOMException in first-party code, which does carry caller frames." For deadline signals, only the pre-Baseline-2024 fallback path in `src/services/timeout-signal.ts` mints such a reason at all; every current engine takes the native `AbortSignal.timeout` branch. Even that fallback does not reliably carry caller frames. It builds a bare DOMException, which Chromium-family engines leave stackless, so it gets frames only if Sentry's fetch backfill writes the call site onto it. Engines that do record a DOMException stack give it the timer callback's frames, not the caller's. It has not yet been given the native header-only stack. Believing the comment is why the gate was written to suppress the shape in the first place. (One hand-built reason did reach first-party frames on every engine by another route: the insights loader's own `signal timed out` DOMException had no `stack` in Chromium, so Sentry's fetch instrumentation wrote the fetch call site onto it when a browser extension leaked the rejection. PR #8296 now gives it the native header-only stack — see `docs/solutions/logic-errors/sentry-stack-backfill-makes-a-stackless-abort-reason-look-first-party.md`.)
