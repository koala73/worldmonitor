...
- **MCP calls draw on this same daily allowance.** An API plan has one budget covering both REST requests and MCP tool calls; there is no separate MCP allowance to track. An MCP call that answers from cache costs the same as one REST request, and one that fetches live data downstream costs two — see [MCP calls against an API plan](#mcp-calls-against-an-api-plan).
...
Pro and Pro Business have no REST allowance, so their MCP calls are counted separately. API plans have one budget: the "MCP calls / day" column is not an additional allowance on top of the API one, it is the same number.
