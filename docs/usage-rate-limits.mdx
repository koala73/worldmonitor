...
## Upstream-backed read endpoints

These read endpoints reach a metered third party, so they carry a tighter budget than the 600/60 s default. Plan pacing against these numbers rather than the default.

| Endpoint | Limit | Window | Scope |
|----------|-------|--------|-------|
| `GET /api/youtube/live` | 60 | 60 s | Per IP |
| `GET /api/reverse-geocode` | 120 | 60 s | Per IP (charged on cache misses only) |

Matches the implementations in `api/youtube/live.js` and `api/reverse-geocode.js`. Both fail open: if the rate-limit store is unavailable the request is allowed rather than rejected, so a 429 always means the budget was genuinely exceeded.
