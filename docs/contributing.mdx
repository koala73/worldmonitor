...
World Monitor does not currently ship a publishable CLI package or a tag-driven CLI publish workflow in this repository. Before maintainers cut a `cli-vX.Y.Z` release, they should wire those pieces up explicitly and verify the release path end to end.

Use this preflight checklist before documenting or automating CLI releases:

1. Add the actual CLI package and make sure it is publishable. The current root `package.json` is marked `private`, so it cannot be published directly.
2. Add the release workflow that should react to `cli-vX.Y.Z` tags and publish the CLI package.
3. Confirm `NPM_TOKEN` and any other publish secrets are configured before pushing the release tag.
4. Run `npm ci` and `npm run build` so the release starts from a clean, reproducible install.
5. Make the package version and `cli-vX.Y.Z` tag match exactly, then push the tag only after the release automation exists and secrets are ready.

Until that automation lands, a pushed `cli-vX.Y.Z` tag is only metadata. It will not publish a CLI release by itself.
