...
| `api/v2/shipping` family | **Declared cloud-only**: `scripts/build-sidecar-handlers.mjs` lists it in `CLOUD_ONLY_ROUTE_FAMILIES` and fails the build on any undeclared `api/v{N}/{domain}` family; the sidecar routes `/api/v2/shipping/` to the cloud on purpose (`cloudOnlyRouteFamilyPrefixes`) instead of via the 404 fallback. `tests/sidecar-desktop-path-decisions.test.mjs` requires every `server/worldmonitor/` family to be either bundled or declared | #5907 | `scripts/build-sidecar-handlers.mjs`; `src-tauri/sidecar/local-api-server.mjs`; `src/shared/premium-paths.ts` |
