...
| Runtime detection | **Closed**: every client call site now derives desktop from `isDesktopRuntime()` in `src/services/desktop-runtime.ts`; only the detector and `tauri-bridge.ts` (which needs `invoke`, not a boolean) may read the bridge globals by name. `tests/desktop-runtime-single-detector.test.mjs` scans `src/` for regressions; `tests/dom/desktop-runtime-variant-convergence.test.mts` proves `SITE_VARIANT` honours the stored variant on a bridge-less Tauri origin | #5912 | `src/services/desktop-runtime.ts`; `src/config/variant.ts` |
