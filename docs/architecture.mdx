...
| **Run anywhere**                    | The same codebase produces the published specialized variants from a single Vercel deployment and deploys to Vercel (web), Railway (relay), Tauri (desktop), and PWA (installable). Desktop sidecar mirrors all cloud API handlers locally. Service worker caches map tiles for offline use while keeping intelligence data always-fresh (NetworkOnly).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| **Type-safe data flow**             | Discriminated union markers (`_kind` field), proto-generated typed clients/servers, and exhaustive `switch` matching ensure compile-time safety across marker types, service domains, and map layers. Adding a new data type produces compiler errors at every unhandled site.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
**hreflang policy by surface:**

| Surface | Indexable languages | Declaration |
| --- | --- | --- |
| Apex `/`, `/dashboard`, `/pro`, and English crawlable corpus pages (`/countries/`, `/chokepoints/`, …) | English only | Self-pointing `en` + `x-default` |
| Mintlify docs (`/docs/…` and `/docs/zh/…`) | English + Simplified Chinese | Reciprocal `en` / `zh-Hans` / `x-default` (English is `x-default`) |
| Application UI via `?lang=` | Not indexable alternates | Never advertised in hreflang |

Publishing any *additional* non-English hreflang entry (for example `/fr/` app
routes) requires a separately reviewed locale-prefixed static or
server-rendered architecture. Every advertised page must meet the complete
...
architecture exists for a locale, adding a client translation or a supported
locale code does not authorize publishing a new indexable alternate.
...
Service domains cover every data vertical:
...
| --- | --- | --- | --- |
| `seed-earthquakes` | USGS M4.5+ and Earthquakes Canada (NRCan) Atom | 5 min | `seismology:earthquakes:v1` |
...
| `seed-cyber-threats` | Feodo, URLhaus, ThreatFox, C2Intel, OTX, AbuseIPDB | 2 hours | `cyber:threats-bootstrap:v2` |
| `seed-internet-outages` | Cloudflare Radar | 5 min | `infra:outages:v1` |
| `seed-fire-detections` | NASA FIRMS VIIRS, CWFIS national active/prescribed fires (CWFIF WFS), and BC Wildfire current fire points (OpenMaps KML) | 10 min | `wildfire:fires:v1` |
...
| `seed-military-bases` | Curated military-base registry | Daily | N/A |
...
World Monitor uses Vercel Edge Functions as a lightweight API layer, split into two generations. Legacy endpoints in `api/*.js` each handle a single data source concern — proxying, caching, or transforming external APIs. The newer proto-first endpoints use **per-domain thin entry points**, each importing only its own handler module. This replaced the original monolithic gateway that loaded every domain on each cold start. Each domain's function tree-shakes to include only its dependencies, reducing cold-start time by ~85% (sub-100ms for most endpoints vs. 500ms+ with the monolithic handler). A shared `server/gateway.ts` provides common routing logic. Both generations coexist, with new features built proto-first. This architecture avoids a monolithic backend while keeping API keys server-side:

- **RSS Proxy** — domain-allowlisted proxy for the registered feed catalog, preventing CORS issues and hiding origin servers. The Edge proxy and Railway relay both keep redirects manual and re-check the RSS domain allowlist before every hop. Feeds from domains that block Vercel IPs are automatically routed through the Railway relay.
...
- **Finance Geo Data** — stock exchanges, financial centers, central-bank and supranational finance institutions, and commodity hubs are served as static typed datasets with market caps, GFCI rankings, trading hours, and commodity specializations
...
All published dashboard variants serve from a **single Vercel deployment**. The variant is determined at runtime by hostname detection:
