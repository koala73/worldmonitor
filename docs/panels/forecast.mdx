...
The seeder also publishes horizon projections (`24h`, `7d`, `30d`) from domain-specific projection curves in `scripts/seed-forecasts.mjs`. Each forecast is normalized against the strongest multiplier in its domain curve before the three horizons are expanded, so the emitted probability represents the peak of that curve rather than whichever horizon label the forecast carries. Projected probabilities are then clamped to a 1% floor and 95% cap (`0.01` / `0.95`) as a presentation heuristic, so projection endpoints should be interpreted as bounded scenario paths rather than unconstrained calibrated probabilities.
...
Probability projections are normalized against each domain curve's peak multiplier and then expanded to 24h, 7d, and 30d:
