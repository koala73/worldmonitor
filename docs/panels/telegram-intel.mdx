...
- **Custom public channels** — resolve an `@username`, preview it, and add up to 20 channels. The list is stored locally under `telegram:watchlist:v1`; each channel can be removed from its pill.
...
| Read custom channel | `GET /api/telegram-feed?mode=channel&username=...` | Returns recent public-channel posts through the same credential gate and 30-second client cache as the curated feed. |
