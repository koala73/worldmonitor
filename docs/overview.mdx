...
World Monitor combines structured APIs, public and licensed datasets, official government and multilateral sources, news feeds, and reviewed OSINT. The current generated inventory records 632 configured feed definitions, 529 providers, and 531 active upstream hosts. The Telegram catalog currently enables 56 channels; this count is separate from the RSS and provider inventory.

Representative providers include ACLED, UCDP, GDELT, UNHCR/OCHA, IMF, World Bank, BIS, Eurostat, FRED, USGS, NASA, Open-Meteo, Cloudflare Radar, AIS and ADS-B providers, and hundreds of news publishers. RSS configuration can include source tier, state affiliation, and propaganda-risk metadata. Telegram configuration uses a different schema with tier, topic, and region metadata. Cross-source corroboration can improve ranking and confidence, but it is not a universal prerequisite for every alert.

Use the [Data Sources catalog](/data-sources) to review coverage by domain. Use the machine-generated [Source Attribution inventory](/source-attribution) for the observed upstream-host ledger and its attribution or terms-review posture. Generation checks detect repository drift, but an inventory entry is not by itself a legal clearance. Source availability and licensing can differ by feature and deployment model.
