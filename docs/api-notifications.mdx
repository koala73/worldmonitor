...
The `origin` and `parentOrigin` parameters accept enumerated app origins, team-pinned Vercel previews, and local development or Tauri origins. Vendor subdomains such as `clerk.worldmonitor.app` and `abacus.worldmonitor.app` are excluded.
