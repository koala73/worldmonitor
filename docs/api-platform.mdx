...
Reverse geocodes a lat/lon to the nearest country + city. Results are cached server-side for 7 days on a 0.1-degree grid; a cache miss is resolved upstream via Nominatim.

**Rate limit:** 120 requests/minute per IP, charged on cache misses only (a cache hit never reaches the upstream geocoder). A 429 carries the standard `RateLimit-*` and `Retry-After` headers.
