...
WorldMonitor MCP uses **OAuth 2.1 with PKCE**. A client registers once, exchanges an authorization code for a bearer token, then passes the token on every JSON-RPC call as `Authorization: Bearer <token>`.
...
- **Dynamic registration**: 10 registrations / hour / IP
...
The server exposes 32 tools. Most are cache-reads over pre-seeded Redis keys (sub-second). Five (`get_world_brief`, `get_country_brief`, `analyze_situation`, `generate_forecasts`, `search_flights`, `search_flight_prices_by_date`) call live LLMs or external APIs and are slower.
