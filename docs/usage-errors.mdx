...
| `429` | Rate limited - an authenticated REST key exceeded its per-minute burst limit or daily plan allowance, or another route-specific limit | Yes - honor `Retry-After` |
...
| `Daily request limit reached` | The authenticated REST key exceeded its daily plan allowance. The response has `limit_type: "daily"`; retry after `Retry-After`, which reaches the next UTC midnight. |
...
For authenticated REST API keys, a 429 response identifies the active plan limit with `limit_type`:

- `per_minute` means the per-minute burst limit was exceeded. Retry after the burst window resets.
- `daily` means the daily plan allowance was exceeded. The request at the allowance is accepted; the next request is rejected until the UTC-day reset. It is not a billable overage.

The API sends `Retry-After` on both responses. SDK REST calls surface these responses through their existing API transport errors; see [Official SDKs](/sdks#rest-rate-limit-errors).
