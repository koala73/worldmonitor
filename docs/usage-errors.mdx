...
| `403` | Authenticated but not entitled (usually `pro_required`), or `subscription_lapsed` — a lapse confirmed with the billing provider (`X-Billing-Verification` header set). Also `{"error":"Forbidden"}` from the edge bot gate — see below. | No — upgrade / resubscribe (or send a real User-Agent / API key for the bot gate) |
...
A handful of public paths skip the gate entirely: `/api/version`, `/api/health`, `/api/product-catalog`, `/api/llms.txt`, `/api/seed-contract-probe`. Everything else — including anonymous public reads like `GET /api/bootstrap?public=1` — is behind it, which is why a bare `curl` of a documented anonymous endpoint returns 403 while the same URL works from a browser.
