...
| 200 OK | `{ status: "composing" }` — no brief for the current/requested slot yet |
| 401 | Missing / invalid Clerk JWT |
| 403 | `pro_required` |
| 503 | `BRIEF_URL_SIGNING_SECRET` not configured |

The `magazineUrl` is a freshly-signed URL — the HMAC binds `{userId, issueSlot}` so it only works for the authenticated owner.
