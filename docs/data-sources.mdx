...
- IMD RSMC New Delhi cyclone tracks, forecast wind radii, cone of uncertainty, and port / sea-area / coastal bulletins (`weather:imd-cyclone-marine:v1`, on-demand). Live fetch stays disabled until API-key and redistribution rights are confirmed; a disabled snapshot is not an all-clear.
