...
Compare the dashboard with its underlying event-data services: [World Monitor vs ACLED](https://www.worldmonitor.app/compare/worldmonitor-vs-acled/) and [World Monitor vs GDELT](https://www.worldmonitor.app/compare/worldmonitor-vs-gdelt/).
...
Feeds are assigned a source tier reflecting editorial reliability:

| Tier       | Description                                 | Examples                                             |
| ---------- | ------------------------------------------- | ---------------------------------------------------- |
| **Tier 1** | Wire services and official bodies           | Reuters, AP News, AFP, Pentagon                      |
| **Tier 2** | Major outlets                               | BBC, CNN World, New York Times, The Guardian, Al Jazeera |
| **Tier 3** | Specialist, regional and think-tank sources | Defense One, Breaking Defense, The War Zone          |
| **Tier 4** | Aggregators and blogs                       | Hacker News, The Verge, Yahoo Finance                |

A clustered story lists its publishers with the best tier declared among the feeds seen for that story. A feed with no declared tier shows as "tier not declared" and never counts as tier 4 when the corroboration flags are computed. Tiers rank sources; they do not judge whether a claim is true.
...
Seven threat intelligence feeds provide indicators of compromise (IOCs) for active command-and-control servers, malware distribution hosts, phishing campaigns, malicious URLs, and ransomware operations:
...
| **ThreatFox** (abuse.ch)     | Mixed         | Community-shared IOC database   |
