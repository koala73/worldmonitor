...
- **Pro intelligence suite — nine derived datasets** (#6436, #6437, #6438, #6439, #6440, #6441, #6448, #6449, #6507). Each ships a daily or annual seeded snapshot, a REST route, and (except the vulnerability ranking list) an MCP tool: physical-vs-paper precious metals — the SGE/COMEX premium series and the divergence index computed over its bounded history; mineral production and processing concentration by country and stage, with HHI, from USGS MCS with BGS fill; the defense industrial base — World Bank `MS.MIL.*` capacity indicators plus SIPRI-derived five-year arms-supplier shares and concentration; demographics capability across UN WPP age structure, the UNESCO UIS / World Bank WDI education pipeline, and ILOSTAT industrial workforce; food stocks and stocks-to-use per commodity-country from USDA PSD with a FAOSTAT production-only fill; indicator-level resilience explainability across all 72 registered indicators with runtime weights and reconciled contributions; commodity supply vulnerability per country plus the chokepoint dependency fan-out; and the five-factor country and bloc scorecard over food, energy, demographics, technology and defense. Every response distinguishes "measured zero" from "no data" through an explicit flag (`available`, `hasScore`, `hasValue`, `state`), so a numeric zero is never sufficient on its own. Full usage, per-dataset traps, and the methodology pages are collected in [Pro Intelligence Suite](/pro-intelligence-suite).
- **Widescreen split layout on the web** (#6417): the map-beside-panels layout now activates at 900px viewport width (was 1600px), matching the desktop app, with the threshold defined once in `src/app/split-layout.ts`. The map column can narrow to 220px (was 25% minimum) and the panels column keeps a 300px floor; the width divider gains a tooltip, touch drag, and RTL-aware keyboard resize; a new map header toggle moves the map to the right edge (persisted); map heights are stored per layout mode so the stacked and split layouts no longer overwrite each other's saved height
...
- **Seven intelligence-suite routes now require Pro** (#7433; #6436, #6438, #6439, #6448, #6449). **This removes free access that previously worked.** `GET /api/market/v1/get-physical-premiums`, `/api/market/v1/get-physical-divergence-index`, `/api/supply-chain/v1/get-mineral-production`, `/api/military/v1/get-defense-industrial-base`, `/api/supply-chain/v1/get-country-vulnerabilities`, `/api/supply-chain/v1/get-chokepoint-dependencies` and `/api/supply-chain/v1/list-vulnerability-rankings` shipped ungated and served full payloads to anonymous callers; all seven are now entitlement tier 1. Anonymous callers get `401`; a signed-in free account gets `403` with `error.data.reason = "upgrade-required"` and an `upgradeUrl`. **Neither is retryable** — only a `503` carrying `X-Billing-Verification` is. In the dashboard, free viewers now see an upgrade card in place of the defense-industrial and commodity-vulnerability cards in the country deep dive and the dependency block in the Hormuz tracker; the Supply Chain minerals tab falls back to the free deposits table with a note; and the commodities **Physical** tab no longer appears at all, because the tab is built only when premium data is present. `/api/military/v1/get-defense-industrial-base` additionally lost its anonymous `public=1` shared-cache shape, which had bypassed the entitlement check entirely — so it no longer has a CDN-shared response. One path is deliberately unchanged: the `get_market_data` MCP tool is a cache-backed bundle whose datasets include `physical-premium` and `physical-divergence`, and it remains callable by signed-in free accounts within their daily allowance. See [Pro Intelligence Suite](/pro-intelligence-suite) for the access contract.
...
- **Gateway POST→GET compatibility no longer drops nested body fields** (#7276). The legacy unmatched-POST fallback used to copy only scalars and scalar array members into query parameters and ignore the rest, so a stale client could get a successful unfiltered GET while believing its object filters were applied. Mixed, nested, or non-object JSON bodies now return HTTP 400 without a partial translation. Malformed JSON also returns 400 (`Invalid JSON body for POST compatibility`). Empty or whitespace-only bodies still convert to GET with no extra query parameters. The existing 1 MB body cap and 200-values-per-key cap are unchanged.
