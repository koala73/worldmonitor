...
The desktop app is built from the World Monitor source and **the binary is licensed to you under AGPL-3.0-only** — run it, study it, modify it, redistribute it under the AGPL's terms. Source: [github.com/koala73/worldmonitor](https://github.com/koala73/worldmonitor). Every packaged build ships `resources/notices/THIRD-PARTY-NOTICES.md`, generated at build time, with the verbatim notices for the npm packages and Rust crates it contains; the bundled Node.js runtime carries its own `LICENSE` alongside it.

Running the app entirely against your own API keys and local sources needs no plan from us. The moment it fetches World Monitor data — cloud fallback, entitlement checks, or our API — that use is governed by the [End User License Agreement](/eula), which covers desktop and web as one license rather than two.

## Overview

### Desktop Application (Tauri)

- **Native desktop app** for macOS, Windows, and Linux — packages the full dashboard with a local Node.js sidecar that runs the supported API handler catalog locally
- **OS keychain integration** — API keys stored in the system credential manager (macOS Keychain, Windows Credential Manager, Linux Secret Service), never in plaintext files. Linux uses the Secret Service API through the same `keyring` crate; a Linux desktop without an activatable Secret Service provider currently starts with an empty vault (#802).
...
- **Settings window** — dedicated configuration UI (Cmd+,) with three tabs: **LLMs** (Ollama endpoint, model selection, Groq, OpenRouter), **API Keys** (the supported data-source credential catalog with per-key validation), and **Debug & Logs** (traffic log, verbose mode, log files). Each tab runs an independent verification pipeline — saving in the LLMs tab doesn't block API Keys validation
...
All published variants run on three platforms that work together:
...
**Why two platforms?** Several upstream APIs (OpenSky Network, CNN RSS, UN News, CISA, IAEA) actively block requests from Vercel's IP ranges, and some require persistent connections or protocols that edge functions cannot support. The [Railway relay server](/relay-parameters) acts as an alternate origin, handling:
...
- **Telegram intelligence** — a GramJS MTProto client polls the reviewed OSINT channel registry on a 60-second cycle with per-channel timeouts and FLOOD_WAIT handling
...
API keys are stored in the operating system's credential manager (macOS Keychain, Windows Credential Manager, Linux Secret Service) — never in plaintext config files. Linux uses the Secret Service API through the same `keyring` crate; a Linux desktop without an activatable Secret Service provider currently starts with an empty vault (#802). All secrets are consolidated into a single JSON vault entry in the keychain, so app startup requires exactly one OS authorization prompt regardless of how many keys are configured.
...
The sidecar includes a built-in RSS proxy handler that fetches the registered news feeds directly from source domains, bypassing the cloud RSS proxy entirely. This means the desktop app can load the complete feed catalog without any cloud dependency — the same domain allowlist used by the Vercel edge proxy is enforced locally. Combined with the local API handlers, this enables the desktop app to operate as a fully self-contained intelligence aggregation platform.
...
The desktop app checks for new versions by polling `api.worldmonitor.app/api/version` — once at startup (after a 5-second delay) and then every 6 hours. When a newer version is detected (semver comparison), a non-intrusive update badge appears with a direct link to the GitHub Release page.
...
The `/api/version` endpoint reads the latest GitHub Release tag and caches the result at the edge for 5 minutes (`s-maxage=300`, served stale for up to 60 s while revalidating and for up to an hour if GitHub errors), so version checks don't hit the GitHub API on every request.
