...
The desktop app is built from the World Monitor source and **the binary is licensed to you under AGPL-3.0-only** — run it, study it, modify it, redistribute it under the AGPL's terms. Source: [github.com/koala73/worldmonitor](https://github.com/koala73/worldmonitor). Every packaged build ships `THIRD-PARTY-NOTICES.md` in its resources directory, generated at build time, with the verbatim notices for the npm packages and Rust crates it contains; the bundled Node.js runtime carries its own `LICENSE` alongside it.
