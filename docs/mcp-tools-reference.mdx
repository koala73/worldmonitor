...
| `min_domains` | number | Distinct signal domains required per cell, 2-5 (default 3). |
...
Focal-point detection: entities where news coverage and live map signals converge, ranked by multi-signal score. News story clusters are entity-matched against the curated registry, cross-referenced with cross-source escalation signals, and scored with the same engine the dashboard runs. Includes an `aiContext` block and mapping-coverage counters.
...
| `limit` | number | Cap the focal point list (default 10). |
...
curl -s https://worldmonitor.app/mcp \
  ...
  -d '{
    "jsonrpc":"2.0","id":1,
    "method":"tools/call",
    "params":{"name":"simulate_infrastructure_cascade","arguments":{"source_id":"hormuz","disruption_level":0.8}}
  }'
```
...
| `radius_km` | number | Radius in km for point mode (default 50). |
| `limit` | number | Cap the enriched event list in events mode (default 20). |
...
| `limit` | number | Cap the ranked hotspot list (default 29, the full curated set). |
