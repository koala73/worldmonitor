...
Most tools in this section perform live LLM work; `get_world_brief` is the cache-backed exception and returns the precomputed, citation-grounded dashboard snapshot.

### `get_world_brief`

Citation-grounded world intelligence brief from the same precomputed `news:insights:v1` snapshot the dashboard renders. The MCP call does not run a per-call LLM. `geo_context` remains accepted for compatibility but does not narrow or regenerate this global snapshot.

**Parameters:**

| Name | Type | Required | Description |
|------|------|---------:|-------------|
| `geo_context` | string | no | Legacy compatibility input; ignored for this global precomputed snapshot and does not focus or regenerate the brief |

- **Source:** canonical seeded Redis key `news:insights:v1`, shared with dashboard bootstrap hydration.
- **Kind:** cache-backed RPC — returns the producer's citation-locked brief, story lines, and source records; a missing or malformed snapshot fails closed rather than falling back to an ungated live summary.
- **Freshness:** bounded by the insights seeder cadence; `generatedAt` and `status` are returned with the brief.
