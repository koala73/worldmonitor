...
```json
{
  ...
  "properties": {
    ...
    "upstreamUnavailable": { "type": "boolean", "description": "True when every upstream read failed. The zeroed risk fields then mean UNKNOWN, not low." }
  }
}
```
...
type CountryRisk = {
  ...
  fetchedAt: number;
  // Check this BEFORE reading any score: true means every upstream read
  // failed, so the zeroed fields are unknown, not calm.
