...
asserted field-for-field against this config rather than left to a comment. An
MCP caller still sees a single boolean — `stale` does not say *which* dimension
failed, so `/api/health` remains the surface that names the stale country.
