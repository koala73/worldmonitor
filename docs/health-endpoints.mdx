...
The commodity-vulnerability cohort is stricter than a non-empty check. Its country projection must meet the configured country floor and preserve the full reviewed commodity, HS4, and transit-HS2 registry coverage. The inverse projection must meet its configured chokepoint floor. Any shortfall reports `COVERAGE_PARTIAL`; before the first atomic cohort activation, both checks remain rollout-pending.
