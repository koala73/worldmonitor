...
Every one of the eight coverage checks also carries `activated` (true once its durable marker exists) in **all** statuses, including after that market's rollout window has been pruned, so an `EMPTY` market can be told apart as "activated, then broke" rather than "never ran" without probing Redis by hand.
