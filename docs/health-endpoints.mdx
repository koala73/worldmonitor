...
The commodity-vulnerability cohort is stricter than a non-empty check. The authoritative cohort pointer read by every vulnerability RPC must exist. Its country projection must meet the configured country floor, every covered country must carry all reviewed mapping rows, and the run must observe the full reviewed commodity, HS4, and transit-HS2 input sets. Registry constants alone do not satisfy these measured coverage checks. The inverse projection must meet its configured chokepoint floor. Any shortfall reports `COVERAGE_PARTIAL`; before the first atomic cohort activation, both checks remain rollout-pending.
