...
`STALE_SEED`. A miss with nothing left to serve skips all of this and writes
`status: "error"` immediately, and a failure streak never outranks a missing
data key, which is still `EMPTY` (crit).
