...
evicted, renamed, or unreadable marker would otherwise disable this alarm
permanently.
...
read-and-absent result, so there is no input class on which they answer
differently — a single test loop asserts every marker outcome against every
block shape on all three at once.
