...
Health pins that set in its own config rather than accepting whatever the
producer declares, so a producer-side change cannot narrow the alarm scope
silently: dropping `CN` from the seeder's list would otherwise report `OK` with
China days stale, which is the exact failure this check exists to catch. A
producer set that covers the pinned countries and adds more is accepted.
