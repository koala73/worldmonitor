...
`china-corridor-source-adapters.ts` applies to a PortWatch observation. Health
is inclusive at the boundary and treats a future-dated observation as stale,
both one step more conservative than the data gate — an alarm should fire no
later than the contract it protects.
