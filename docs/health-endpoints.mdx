...
| `RELAY_GATE_MISCONFIGURED` | Crit | The production build serving `/api/health` cannot see `CONVEX_SITE_URL` or `CONVEX_TENANT_RELAY_SECRET`, so checkout, the customer portal and notification channels answer 503 before reaching Convex. Reported on the `relayGatewayGate` liveness entry, which is not a data key and is not part of `summary.total`. Set the value, then push a new commit; a same-commit redeploy is cancelled by the ignored-build step |
...
| `RELAY_GATE_UNREACHABLE` | Warn | The gate probe got no classifiable answer (network error, timeout, 5xx, or anything other than `401` / `400 MISSING_FIELDS`). A transport verdict, not a credential one; persistent across sweeps means a Convex outage or a wrong `CONVEX_SITE_URL` |
