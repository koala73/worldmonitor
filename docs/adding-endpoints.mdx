...
- [ ] If `fetch-on-miss`, name the upstream cost, cardinality, and tier policy that makes open MCP exposure unsafe for now.
...
| `fetch-on-miss` | The operation can call a paid, rate-limited, high-cardinality, or otherwise expensive upstream when the cache is cold. Include one of the enforced secondary signals in the reason. |
