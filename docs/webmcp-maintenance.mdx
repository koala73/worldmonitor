...
The headed production suite tests the real origin trial. It verifies the served SHA, security headers, twenty-seven-tool inventory and schemas, cold-start invocation, bounded access and sign-in results, read-only catalogs, cancellation behavior, all six dashboard origins, specialized-root redirects, and cross-origin embed denial. Mutation tests that would alter production state remain local.
