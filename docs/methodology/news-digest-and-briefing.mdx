...
flashpoint keyword. A low/medium item can also be promoted to `high` when it is
not historical, has a diplomacy/flashpoint signal, and has at least `3`
Tier 1 or Tier 2 entity-level sources.
...
| `fading` | Current score is less than half the peak score. |
...
batch falls back to Jaccard. Topic grouping is enabled by default after dedupe
