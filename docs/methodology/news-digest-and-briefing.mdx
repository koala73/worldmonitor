...
context. A per-story `whyMatters` line, from the analyst endpoint or the direct
fallback, has any tenure qualifier the story does not carry (such as "former"
or "acting" before a titled name) removed, and is rejected if one remains.
