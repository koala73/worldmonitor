...
The per-metal stress index is on a 0 to 100 scale. Absolute magnitude interpolates across compressed band tops of 45 (elevated), 70 (stressed), and 90 (just under extreme). Clearing the absolute extreme premium floor (gold 5%, silver 20%) publishes `100`.
...
| `physical-divergence-v1` | 2026-08-30 | Index floors compressed to 0 / 45 / 70 / 90; absolute band tops nest under those floors; `100` reserved for absolute extreme so a relative-only extreme cannot saturate the scale (#7423). |
