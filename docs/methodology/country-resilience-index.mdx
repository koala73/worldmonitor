...
Countries with very sparse overall coverage below **0.40** still trigger the
legacy grey-out treatment in UI surfaces. Public ranking inclusion, however,
is governed by the stricter server-side `headlineEligible` gate:
