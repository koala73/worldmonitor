...
description: "Real-time resilience scoring for 196 countries across 6 domains and 21 dimensions — 0-100 index refreshed every 6 hours at OECD/JRC parity."
---
...
|---|---|---|---|---|
| `resilience:score:v27:{countryCode}` | JSON | 6 hours | `buildResilienceScore` in `server/worldmonitor/resilience/v1/_shared.ts` | `getResilienceScore` handler |
| `resilience:ranking:v27` | JSON | 12 hours | `getResilienceRanking` warm path, only when at least 90% of countries are scored (`RANKING_CACHE_MIN_COVERAGE = 0.90`) | `getResilienceRanking` handler |
| `resilience:history:v21:{countryCode}` | sorted set | indefinite, trimmed to 30 days | `appendHistory` during scoring | trend and `change30d` computation |
| `resilience:intervals:v10:{countryCode}` | JSON | 7 days; freshness monitored via `seed-meta`/health cadence | `scripts/seed-resilience-scores.mjs` | `getResilienceScore` (optional `scoreInterval` field) and `getResilienceRanking` (`rankStable`) |
...
Rollback: set `RESILIENCE_PILLAR_COMBINE_ENABLED=false`, flush the current `resilience:score:v27:*`, `resilience:ranking:v27`, and `resilience:history:v21:*` keys (or wait for TTLs to expire). The 6-domain formula lives alongside the pillar combine in `_shared.ts` and needs no code change to come back.
...
- **Dark-dimension reconciliation.** `financialSystemExposure` remains outside `RESILIENCE_FLAG_DARK_WHEN_ZERO_COVERAGE` and is still dark. Folding it in would change `overallCoverage` — and therefore `headlineEligible` — for every country, which needs its own measurement and cache-generation reasoning rather than riding along with a different dimension's activation. #6459 explicitly left its activation as a separate phase.
