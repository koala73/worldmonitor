...
- **`cyberDigital` discovery-day smoothing (#4009).** Same-discovery-day burst volatility remains bounded by the severity-weight cap above. With #4008's stable `firstSeenAt`, the scorer now groups threats by discovery age, caps each discovery-day bucket at 8, applies a one-day half-life to older buckets, and caps the summed cyber sub-component at 8. A single burst therefore fades as the feed ages, while repeated discoveries across multiple days can still reach the full capped penalty. Missing or malformed `firstSeenAt` values fall back to the current-snapshot bucket so legacy rows remain bounded rather than silently disappearing. This is scorer-side discovery-day smoothing, not a separate resilience-owned EWMA key.
