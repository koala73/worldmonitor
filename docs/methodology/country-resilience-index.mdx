...
For the live deployment state, use the [runtime manifest](/api/resilience/v1/get-runtime-manifest). It reports the active formula tag, static `dataVersion`, and ranking-cache metadata without exposing deploy identifiers, raw `RESILIENCE_*` flag states, or internal cache keys. The Reference Edition remains a frozen shipped bundle; it should not be treated as proof of the runtime state currently active in production.
