...
For the live deployment state, use the [runtime manifest](/api/resilience/v1/get-runtime-manifest). It reports the deployed SHA, active formula tag, `RESILIENCE_*` flag states, cache identifiers, static `dataVersion`, and ranking-cache metadata. The Reference Edition remains a frozen shipped bundle; it should not be treated as proof of the flags or cache keys currently active in production.
...
Domain scores are first regrouped into the three pillars (`structural-readiness`, `live-shock-exposure`, and `recovery-capacity`) with weights 0.40 / 0.35 / 0.25. The penalty term is anchored to the weakest pillar, so an otherwise strong country cannot fully compensate for a severe weakness in one pillar. The legacy six-domain weighted aggregate remains the rollback formula when `RESILIENCE_PILLAR_COMBINE_ENABLED=false`; an earlier multiplicative form (`baseline * (1 - stressFactor)`) over-penalized every country and was reverted. See the [Changelog](#changelog) for the full version history.
...
|---|---|---|---|---|
| `resilience:score:v19:{countryCode}` | JSON | 6 hours | `buildResilienceScore` in `server/worldmonitor/resilience/v1/_shared.ts` | `getResilienceScore` handler |
| `resilience:ranking:v19` | JSON | 12 hours | `getResilienceRanking` warm path, only when at least 75% of countries are scored (`RANKING_CACHE_MIN_COVERAGE = 0.75`) | `getResilienceRanking` handler |
| `resilience:history:v14:{countryCode}` | sorted set | indefinite, trimmed to 30 days | `appendHistory` during scoring | trend and `change30d` computation |
...
**v17.2 — `cyberDigital` single-day burst cap (#3971).** The scorer now groups matched cyber threats by observed day (`lastSeenAt` first, then `firstSeenAt`) and caps each country-day at severity-weight 8 before applying the existing `normalizeLowerBetter(weightedCount, 0, 25)` transform. Undated legacy items share one deterministic capped bucket. This bounds a single bad feed day while still letting sustained multi-day cyber pressure accumulate. The fix ships with a cache-prefix bump (`v18` → `v19` for both `resilience:score:` and `resilience:ranking:`, plus `v13` → `v14` for `resilience:history:`) so cached uncapped scores and 30-day trend points do not mix with capped scores.
...
- **`cyberDigital` feed availability.** Seeder TTL and last-good behavior protect against empty or failed fetches, but they are availability controls, not score smoothing. Score volatility from same-day feed bursts is bounded in the scorer by the country-day cap above; longer multi-day cyber pressure remains visible by design.
...
4. **Re-anchored release-gate bands**: `tests/resilience-pillar-combine-activation.test.mts` pins high-band anchors (NO, CH, DK) at ≥ 55 (vs the 6-domain formula's ≥ 70 floor) and low-band anchors (YE, SO) at ≤ 40 (vs ≤ 45). The snapshot test reads `methodologyFormula` from each snapshot and applies the matching bands. The live full-universe capture confirms the bands hold with margin: NO = 75.40 (≥ 55 by 20.4 points), YE = 29.51 (≤ 40 by 10.49 points).
5. **Projected and authoritative snapshots**: `docs/snapshots/resilience-ranking-pillar-combined-projected-2026-04-21.json` carries the 52-country preview tables used before activation. `docs/snapshots/resilience-ranking-2026-05-28.json` carries the authoritative full-universe live capture after the flag was activated.

Rollback: set `RESILIENCE_PILLAR_COMBINE_ENABLED=false`, flush the current `resilience:score:v19:*`, `resilience:ranking:v19`, and `resilience:history:v14:*` keys (or wait for TTLs to expire). The 6-domain formula lives alongside the pillar combine in `_shared.ts` and needs no code change to come back.
