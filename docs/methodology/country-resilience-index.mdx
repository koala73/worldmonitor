...
1. **`electricityConsumption` is a wealth proxy, not a resilience signal.** Weight 0.30 on the `energy` dimension; rewards per-capita load rather than grid-integrity capacity. Replaced in PR 1 with `powerLossesPct`, `reserveMarginPct`, and `accessToElectricityPct` (the last moved to the `infrastructure` domain). **Status:** PR 1 lands the v2 construct behind the `RESILIENCE_ENERGY_V2_ENABLED` flag (default off); the indicator set is documented under the Energy Domain section below.
...
| powerLossesPct | Electric power transmission + distribution losses (`EG.ELC.LOSS.ZS`). Direct grid-integrity measure | Lower is better | 25 - 3 | 0.10 | World Bank | Annual |
| reserveMarginPct | Generation reserve margin (IEA electricity balance, imputed where unobserved) | Higher is better | 5 - 25 | 0.10 | IEA | Annual |
...
- **Indicators added:** `importedFossilDependence` (`EG.IMP.CONS.ZS × EG.ELC.FOSL.ZS`), `lowCarbonGenerationShare` (`EG.ELC.NUCL.ZS + EG.ELC.RNEW.ZS`), `powerLossesPct` (`EG.ELC.LOSS.ZS`), `reserveMarginPct` (IEA electricity balance). `accessToElectricityPct` moves to the `infrastructure` domain where it acts as a grid-collapse threshold.
- **New seeders (weekly):** `seed-low-carbon-generation.mjs`, `seed-net-energy-imports.mjs`, `seed-power-reliability.mjs`. Each seeder is health-tracked; the indicators ship `imputationClass: 'unmonitored'` with ON_DEMAND_KEYS status until Railway cron provisions and the acceptance-gate reruns pass.
