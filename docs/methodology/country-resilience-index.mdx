...
Added 2026-08-10. Ships **flag-gated dark** behind `RESILIENCE_EDUCATION_ENABLED` (defaults off) at `tier='experimental'`, pending the Railway seeder rollout. While dark the scorer returns the empty-data shape and the dimension contributes nothing to any published score.
...
|---|---|---|---|---|
| `resilience:score:v26:{countryCode}` | JSON | 6 hours | `buildResilienceScore` in `server/worldmonitor/resilience/v1/_shared.ts` | `getResilienceScore` handler |
| `resilience:ranking:v26` | JSON | 12 hours | `getResilienceRanking` warm path, only when at least 90% of countries are scored (`RANKING_CACHE_MIN_COVERAGE = 0.90`) | `getResilienceRanking` handler |
...
Rollback: set `RESILIENCE_PILLAR_COMBINE_ENABLED=false`, flush the current `resilience:score:v26:*`, `resilience:ranking:v26`, and `resilience:history:v21:*` keys (or wait for TTLs to expire). The 6-domain formula lives alongside the pillar combine in `_shared.ts` and needs no code change to come back.
