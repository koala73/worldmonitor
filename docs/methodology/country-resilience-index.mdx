...
inventory. Two labels appear there and they do not mean the same thing:

- **Observed** — the snapshot holds a usable series for that dimension. The
  inventory applies this label to any dimension that is neither a coverage gap
  nor marked not-applicable.
- **Supported reading** — the dimension additionally clears **0.50** coverage,
  so the page publishes its score next to the label.

A dimension between those two states is recorded as observed without appearing
among the supported readings. The page names those dimensions and states the
0.50 floor inline, so the two lists never appear to contradict each other.

The inventory itself is capped at the 12 lowest-coverage dimensions. That
splits the active dimensions into three buckets, and the scope note accounts
for all three: the dimensions shown, the dimensions already at full coverage
that never entered the weakest-first pool, and the dimensions the cap omitted.
