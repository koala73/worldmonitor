...
Every indicator in the scorer passes a single **mechanism test**: *what direct shock channel does this measure?* If the only answer is "this country is rich," the indicator is excluded from the core score regardless of its historical correlation with resilience outcomes. If the answer is "capacity X absorbs shock Y," the indicator can enter but must use a threshold or saturating transform so it rewards the mechanism rather than the level of resource that drives it.

The mechanism-test classification per indicator is documented in `docs/methodology/indicator-sources.yaml`. The repair plan in `docs/plans/2026-04-22-001-fix-resilience-scorer-structural-bias-plan.md` identifies the indicators that currently fail the test (tagged `wealth-proxy`) and the sequenced PR work to replace them.
