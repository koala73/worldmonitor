...
|---|---|---|---|---|
| `resilience:score:v7:{countryCode}` | JSON | 6 hours | `buildResilienceScore` in `server/worldmonitor/resilience/v1/_shared.ts` | `getResilienceScore` handler |
...
**Current published version.** Scored on domain-weighted average of 5 domains and 13 dimensions.
...
- Phase 1 of the reference-grade upgrade plan:
  - T1.1: regression test pins top-of-ranking ordering (Norway elite tier > US strong tier, neither pinned at 100) after investigation of an origin-document claim that did not reproduce.
  - T1.4: widget surfaces `dataVersion` in the footer so analysts can see how fresh the underlying source data is.
  - T1.7: formal four-class imputation taxonomy (`stable-absence`, `unmonitored`, `source-failure`, `not-applicable`) tagged on every absence-based entry in the `IMPUTATION` and `IMPUTE` tables.
