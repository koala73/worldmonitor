...
dropped proxy with structural sanctions exposure via BIS Locational Banking
Statistics + WB IDS short-term external debt + FATF AML/CFT listing status.
...
the BIS LBS, FATF listing, and WB external-debt seeders are provisioned and
...
| `unmonitored` | The source is a curated list that may not cover every country. Absence is ambiguous; penalized conservatively. | 50 to 60 | 0.3 to 0.4 | BIS / WTO / FATF curated feeds and other partial-coverage enrichments |
...
| BIS | Locational Banking Statistics for `financialSystemExposure`; REER remains experimental enrichment / rollback support | Quarterly / monthly | LBS broad coverage; REER ~60 countries |
