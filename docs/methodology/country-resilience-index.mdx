...
- **Acceptance gates.** Spearman must be at least 0.85, at least 60% of countries must move by less than 3 overall points, and no non-sanctions country may move by more than 12 points. Headline-eligibility changes and source-failure caveats remain explicit in the artifact.
...
See [`financial-system-exposure-flag-flip-runbook.md`](./financial-system-exposure-flag-flip-runbook.md) and the committed `resilience-financial-system-exposure-acceptance-{date}.json` snapshot for the operator procedure and measured result.
