...
Promotion from `watch` to `confirmed` needs corroboration from genuinely separate
outlets — two outlets plus a regulator, three outlets, or a regulator plus a hard
operational signal like a cancelled flight. Ten stories rewriting one wire report
do not add up to confirmation.

<Note>
**No human sits in this loop.** Every classifier here runs automatically. Quality
comes from the evidence thresholds above, a second-pass language-model check that
can veto a promotion, and automatic decay of stale rows — not from a review
queue. An alert that stops being refreshed expires on its own rather than sitting
