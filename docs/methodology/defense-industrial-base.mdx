...
`supplierRetained: true` means this importer's current SIPRI request failed and the previously stored row was kept rather than dropped. `supplierFetchedAt` then reports the retained row's original timestamp, not the failed attempt. A retained row is real data with a known age, which is why it is served instead of an empty response.

## Clocks

`fetchedAt` is the **oldest** source timestamp among the values served, so it is safe to treat as "nothing here is newer than this". `industrialFetchedAt` and `supplierFetchedAt` give the per-part clocks. The World Bank and SIPRI parts update on different schedules — SIPRI's annual update lands each March.
