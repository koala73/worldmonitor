...
6. Rounds the published sub-score and coverage to two and four decimal places,
   respectively. Rounding never changes a band.

Unavailable inputs do not receive a neutral score and do not enter the weighted
mean. A pillar below its floor has `hasScore=false`, `score=null`,
`subScore=null`, and one or more explicit reasons.
...
`resilience:food-stocks:v1`. Tonnes are converted to calorie-equivalent units
with the seeder's frozen commodity conversion table before aggregation.
...
| Arms export share of transfers | 0.30 | higher; `0 -> 0`, `1 -> 100`; exports / (exports + imports) |
