...
[Pro Intelligence Suite](/pro-intelligence-suite) for the request shapes.
...
demographics 150, technology 110, and defense 30. The floors were set from the
196-country production-source audit with headroom for normal source coverage.
The pre-activation production refresh measured 116 scoreable technology
countries; 110 preserves a six-country outage margin without accepting stale or
missing country evidence. Health publishes population and the five pillar counts separately. A
...
is derived from the audited World Bank `EG.IMP.CONS.ZS` import-dependency
observation already carried by the resilience static source:

```text
production TWh = consumption TWh * (1 - net imports percent / 100)
```

Where that observation is Eurostat `nrg_ind_id` rather than World Bank, the
input reports `redistribution-blocked` and the country scores no primary
balance. Eurostat is not audited for raw redistribution of this indicator, and
publishing the derived ratio would expose the raw percentage anyway, since
consumption is published beside it. Restoring those countries requires a source
