...
from the audited World Bank net-energy-imports observation (`EG.IMP.CONS.ZS`)
already carried by the resilience static source:

```text
production TWh = consumption TWh * (1 - net imports percent / 100)
```

Where that import-dependency observation is Eurostat `nrg_ind_id` rather than
World Bank, the input reports `redistribution-blocked` and the country scores no
primary balance. Eurostat is not audited for raw redistribution of this
indicator, and publishing the derived ratio would expose the raw percentage
anyway, since consumption is published beside it. Restoring those countries
requires a source audit, not a code change.
