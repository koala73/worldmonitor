...
| **403** | JSON-RPC envelope         | `-32002`         | Terminal entitlement denial: `data.reason: "lapsed-subscription"` (confirmed lapse; also sets `X-Billing-Verification: subscription_lapsed`) or `"upgrade-required"` (free allowance called a Pro-only tool, or a non-free entitlement is inactive). No `WWW-Authenticate` (re-auth cannot fix it). |
