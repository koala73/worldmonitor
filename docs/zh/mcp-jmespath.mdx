---
title: "JMESPath 投影指南"
description: "用 JMESPath 将 MCP 响应载荷缩减 80-95%。基于真实 WorldMonitor 响应结构的十二个实例解析。"
---
...
下面用到的三种响应结构是 `tests/fixtures/jmespath-samples/` 下抓取的固件（会定期对生产 MCP 端点重新抓取）。它们是故意挑选的——大（~100 KB）、中（~30 KB）、小（~10 KB）——以覆盖你实际会投影的各种大小档位。
...
**未投影响应（~100 KB）。** 数百条股票报价、每个板块、全部加密货币、每个海湾 ticker、完整的恐慌贪婪明细：
...
**意图。** 从 `get_market_data` 取一份所有股票报价的紧凑表：代码、价格、百分比变化。
...
<Tip>
**`limit` 与切片。** 几个工具（`get_country_macro`、`get_eu_*`、`get_displacement_data`）接受工具级 `limit` 参数，它会在投影**之前**给响应封顶。`limit` 与 JMESPath 可组合：工具级封顶收窄候选集，然后你的 `[0:N]` 切片取前缀。当你想要全部数据并打算用 JMESPath 进一步投影时，传 `limit: 0` 来禁用工具级封顶。
</Tip>
...
**意图。** 从 `get_conflict_events` 只返回被归类为国家间暴力（state-based violence）的 UCDP 事件——最高严重度档位。
...
完全省略 `jmespath` 参数。某些工具（`get_country_macro`、三个 `get_eu_*` 工具、`get_displacement_data`）出于同样原因也把其列表型字段默认封顶在 30 行；当你确实想要整批数据时，传 `limit: 0` 禁用该封顶：
...
`original_keys` 是未投影响应的顶层键——足以让模型在多一次调用里用修正后的表达式重试。一个坏表达式**确实**每次尝试消耗一个每日配额槽；回显的 `original_keys` 正是为了让重试能自我修正而非瞎猜。三种类型（`expression_too_long`、`invalid_expression`、`projection_too_large`）及其精确判别字符串见 [MCP 错误目录](/zh/mcp-error-catalog#_jmespath_error--projection-failed)。

## 投影的互补项

### `summary: true` 标志

每个缓存工具还接受一个通用的 `summary: true` 参数，它返回一个服务端构造的摘要（计数 + 样本）而非完整载荷。在以下情况使用：
