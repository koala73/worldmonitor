...
| `list_mission_presets` | 可选布尔值 `available`；不允许其他属性。 | 只读返回当前监视器下的全部捆绑任务预设。每项使用稳定预设 ID，并描述目标视图、时间范围、面板与图层，不暴露付费内容。包含 active、monitorCompatible、entitled、available 标志，以及 gated 时的稳定 `unavailableReason`。 |
