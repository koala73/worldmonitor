...
3. 完全重新启动 Chrome。
4. 本地启动 WorldMonitor。打开 `/dashboard` 检查含十三个工具的仪表板；不要使用 `/embed`。若要检查含两个工具的静态首页，请先运行 `npm run build:pro`，再打开 `/pro/welcome.html`。本地 Vite 的 `/` 会加载仪表板 SPA，只有生产环境才把 `/` 重写到欢迎页。
...
`openCountryBrief`、`set_map_layers`、`open_search_result`、`select_dashboard_tab`、`create_dashboard_tab`、`rename_dashboard_tab` 与 `delete_dashboard_tab` 则继续被拦截并返回不可用。`openCountryBrief` 可能启动服务器端 LLM 简报生成（`premiumFetch` → `/api/intelligence/v1/get-country-intel-brief` → `callLlm`）；即使浏览器随后取消调用方的 Promise，这次生成仍会消耗调用者的每日额度。后两个图层工具会把地图图层选择持久化到浏览器存储，启用 AIS 图层时还会建立网络连接。仪表板标签页变更会持久化 `worldmonitor-tabs-v1` 以及当前面板工作区。这些影响可能在取消后继续存在，因此只有浏览器提供目标侧信号时，这些工具才可用。
...
六个仪表板变体都注册相同的十三个命令式工具。登录和权益变化不会改变注册集合；每次调用都会重新检查实时状态。

在没有提供目标侧调用信号的浏览器上，可逆的视图状态工具仍会执行：由于此类浏览器无法取消它们，你中止的调用仍会执行完毕，其可见的仪表板变化也会生效。而 `openCountryBrief`、`set_map_layers`、`open_search_result`、`select_dashboard_tab`、`create_dashboard_tab`、`rename_dashboard_tab` 与 `delete_dashboard_tab` 会返回不可用。国家简报工具可能在调用方取消后继续消耗每日 LLM 额度，图层工具会把图层选择持久化并影响本次会话之外的状态，标签页变更则会持久化命名工作区。参见[可用性](#availability)。
...
| `list_dashboard_tabs` | 可选字符串 `cursor`，匹配 `^tab-[a-z0-9]+-[a-z0-9]+$`；不允许其他属性。 | 只读返回仪表板标签页（命名的持久面板工作区）：稳定 id、名称、是否激活、是否可创建，以及上限锁定原因。当 `tabsTruncated` 为 true 时，`tabCount` 是已持久化的工作区总数，本页省略了后续标签页；传入返回的 `nextCursor` 可列出其余项。 |
| `select_dashboard_tab` | 必填字符串 `tabId`，模式 `^tab-[a-z0-9]+-[a-z0-9]+$`；不允许其他属性。 | 激活该工作区。选择已激活的标签页是成功的空操作。 |
| `create_dashboard_tab` | 可选字符串 `name`，长度 1–40；不允许其他属性。 | 创建工作区并激活它。若修剪后的名称已存在，则返回该标签页而不重复创建。达到上限时返回 `tab_cap`。 |
| `rename_dashboard_tab` | 必填字符串 `tabId` 与必填字符串 `name`，长度 1–40；不允许其他属性。 | 按稳定 id 重命名。名称规则与仪表板标签栏相同。改成当前名称是成功的空操作。 |
| `delete_dashboard_tab` | 必填字符串 `tabId` 与必填布尔值 `confirm`；不允许其他属性。 | 仅在 `confirm` 恰好为 `true` 时删除。最后一个标签页不能删除（`last_tab`）。 |
...
| 目标 | 推荐调用 | 必须检查的内容 |
|---|---|---|
| 读取当前工作区的面板/地图状态 | `get_dashboard_context` | 读取返回的变体、地图视图、图层和面板 ID。若 `*Truncated` 字段为 true，不得把缩短后的列表当作完整列表。 |
...
| 列出仪表板工作区 | `list_dashboard_tabs` | 使用返回的标签页 id，不要用显示名称。当 `tabsTruncated` 为 true 时，传入 `nextCursor` 直到列出每个 id。 |
| 切换仪表板工作区 | `list_dashboard_tabs` → `select_dashboard_tab` | 传入当前标签页 id。选择已激活的标签页是成功的空操作。 |
| 创建或复用命名工作区 | `list_dashboard_tabs` → `create_dashboard_tab` | 已存在的名称会返回该标签页。上限或权益锁定返回 `tab_cap`。 |
...
不要猜测面板 ID、图层 ID、标签页 ID、结果键、权益或隐藏数据。先读取当前页面状态，再调用一个受限操作，检查结果和可见效果，然后继续。
...
| 调用返回 `target_cancellation_unsupported` | 浏览器接受了 WebMCP，但没有把调用的 `AbortSignal` 交给页面 | 使用只读工具或可逆视图状态工具。不得绕过 `openCountryBrief`、`set_map_layers`、`open_search_result` 或仪表板标签页变更工具的拒绝。 |
...
| 标签页变更返回 `tab_cap`、`last_tab` 或 `confirmation_required` | 仪表板标签栏也会拒绝同一操作 | 重新列出标签页。使用当前标签页 id。删除需要 `confirm: true`，且不能移除最后一个标签页。 |
...
| `src/app/panel-layout.ts`、`src/services/tab-store.ts` 和 `src/services/dashboard-tab-actions.ts` | 仪表板标签页持久化、名称与 ID 限制，以及 WebMCP 的列出/选择/创建/重命名/删除。 |
