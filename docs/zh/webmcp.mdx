...
| `list_dashboard_tabs` | 空对象；不允许其他属性。 | 只读返回仪表板标签页（命名的持久面板工作区）：稳定 id、名称、是否激活、是否可创建，以及上限锁定原因。当 `tabsTruncated` 为 true 时，`tabCount` 是已持久化的工作区总数，tabs 数组可能省略非激活条目。 |
...
| 列出仪表板工作区 | `list_dashboard_tabs` | 使用返回的标签页 id，不要用显示名称。遵守 `tabsTruncated`。 |
