...
`openCountryBrief`、`set_map_layers`、`select_dashboard_tab`、`create_dashboard_tab`、`rename_dashboard_tab` 与 `delete_dashboard_tab` 会继续被拦截并返回不可用。`openCountryBrief` 可能启动服务器端 LLM 简报生成；`set_map_layers` 会持久化地图图层选择；仪表板标签页变更会持久化 `worldmonitor-tabs-v1` 与实时面板工作区。`open_search_result` 是选择器：视图状态结果可以执行；会持久化、消耗配额或离开当前页面的结果仍被拒绝。这些影响可能在取消后继续存在，因此只有浏览器提供目标侧信号时才可用。只读的 `list_dashboard_tabs` 仍可调用。
...
在没有提供目标侧调用信号的浏览器上，可逆的视图状态工具仍会执行。`openCountryBrief`、`set_map_layers`、`select_dashboard_tab`、`create_dashboard_tab`、`rename_dashboard_tab` 与 `delete_dashboard_tab` 会返回不可用。`open_search_result` 仍会执行视图状态结果，并拒绝持久化、消耗配额或离开当前页面的结果。国家简报可能消耗配额，图层写入和标签页变更会持久化。只读的 `list_dashboard_tabs` 仍会执行。参见[可用性](#availability)。
...
| 调用返回 `target_cancellation_unsupported` | 浏览器接受了 WebMCP，但没有把调用的 `AbortSignal` 交给页面 | 使用只读工具或可逆视图状态工具。不得绕过 `openCountryBrief`、`set_map_layers` 或仪表板标签页变更的拒绝。对于 `open_search_result`，请选择视图状态结果，或等待能够取消持久化工作的宿主。 |
