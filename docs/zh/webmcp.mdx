...
| `list_map_layers` | 可选 `monitor`：`world`、`tech`、`finance`、`commodity`、`energy`、`happy`。可选 `renderer`：`2d` 或 `3d`。可选 `state`：`enabled` 或 `available`。可选 `cursor`，匹配 `^[a-z][A-Za-z0-9_-]*$`，长度 1–30。可选整数 `limit`：1–8（默认 6）。不允许其他属性。 | 分页返回已注册地图图层的规范目录，包括已禁用图层。每一行含稳定 ID、标签、启用状态、监视器可用性、渲染器兼容性、权益，以及 `set_map_layers` 无法启用时的机器可读原因。不会加载地图数据集。 |
...
| 发现地图图层 ID（含已禁用图层） | `list_map_layers` | 分页浏览目录。若有 `nextCursor` 则继续翻页。启用前检查 `available` 和 `reason`。此工具只读，不会加载地图数据集。 |
