...
| `set_panel_enabled` | 必填字符串 `panelId`，长度 1–96，模式 `^[a-z0-9][a-z0-9@_-]*$`；必填布尔值 `enabled`；不允许其他属性。 | 经用户使用的同一设置持久化/应用路径启用或禁用目录面板。返回请求状态、实际状态以及是否发生变更。未知、不兼容、无权益以及达到免费档上限的面板会被拒绝。需要目标侧取消。 |
