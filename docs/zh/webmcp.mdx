...
即便如此，WorldMonitor 仍会在这些版本上执行那些效果已被现有计量或同意机制覆盖、或以用户可自行覆盖的可见变化呈现的工具——门禁存在的目的是阻止一次无法取消的调用在任何此类机制之外静默持久化，而不是阻止一切有成本或有可见效果的工具。`openCountryBrief`、`openSearch`、`open_dashboard_panel` 与 `set_map_view` 均可正常使用。`openCountryBrief` 会触发一次服务器端 LLM 简报生成（`premiumFetch` → `/api/intelligence/v1/get-country-intel-brief` → `callLlm`），但该开销由权益层按用户计量：网关会把它计入调用者自己的每日额度（付费 500/天，未验证 50/天），并在超额时退回；因此一次被中止的 `openCountryBrief` 仍会消耗调用者自己额度中的一次简报生成。`set_map_view` 会通过 `history.replaceState` 写入地址栏，生成的分享 URL 与页面复制链接控件生成的相同，并会在刷新时被仪表板还原；这次写入在地址栏中可见，并会被下一次人工移动地图覆盖。

`set_map_layers` 与 `open_search_result` 则继续被拦截并返回不可用：两者都会把地图图层选择持久化到浏览器存储（启用 AIS 图层还会建立网络连接）。此类调用若无法取消，其影响会超出本次会话，而不是停留在屏幕上等待用户撤销，并且把地图移回原处也不会还原已存储的值。当浏览器提供目标侧信号后，它们即可使用。
...
在没有提供目标侧调用信号的浏览器上，改变视图状态的工具仍会执行：由于此类浏览器无法取消它们，你中止的调用仍会执行完毕，其可见的仪表板变化也会生效。而 `set_map_layers` 与 `open_search_result` 在此类浏览器上会返回不可用，因为它们会把图层选择持久化、影响超出本次会话。参见[可用性](#availability)。
