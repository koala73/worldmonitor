...
| 发现地图图层 ID（含已禁用图层） | `list_map_layers` | 分页浏览目录。若有 `nextCursor` 则继续翻页，且仅在同一 `monitor`、`renderer`、`state` 下使用。启用前检查 `available` 和 `reason`；这两个字段是当前页面的 `set_map_layers` 门控，不是请求的监视器。此工具只读，不会加载地图数据集。 |
