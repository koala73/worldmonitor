...
| `persist_failed` | `set_panel_collapsed`、`move_panel` | 本次会话中可见布局已改变，但无法写入 `worldmonitor-panel-collapsed`（折叠）或 `panel-order` 与 `panel-order-bottom-set`（移动）。结果带有 `persisted: false`；不要把该变更视为持久结果。 |
...
有两类失败以 Promise 拒绝而不是受限结果的形式出现。仪表板被销毁时，每个面板布局工具与任务预设工具都会以 `WebMcpToolError` 拒绝，并在消息中给出原因 `app_destroyed`；`list_mission_presets` 也会拒绝格式错误的参数和无法识别的变体，而不是把它们作为结果返回。两者都必须在拒绝路径上处理。
...
当预设的非 map 面板中至少有两个出现在当前变体的默认面板集内时，该预设即与监视器兼容；`list_mission_presets` 将其报告为 `monitorCompatible`。被限制的行会省略 `view` 与 `timeRange`，以便每个监视器的目录都保持在 1,500 字符输出预算之内。有两个原因目前是保留且不可达的：没有任何已发布面板设置布局 `fixed` 标志，因此无法观察到 `panel_fixed`；仪表板也不会把宿主取消能力传入预设目录，因此 `list_mission_presets` 的行永远不会带有 `target_cancellation_unsupported`。
...
| 标签页或布局变更返回 `persist_failed` 且 `persisted: false` | 本次会话中的可见变更已生效，但无法写入其存储键——标签页为 `worldmonitor-tabs-v1`，折叠为 `worldmonitor-panel-collapsed`，移动为 `panel-order` 与 `panel-order-bottom-set` | 不要把结果视为持久结果。请用户释放存储或退出隐私模式，然后重新读取当前状态。 |
