...
3. 完全重新启动 Chrome。
4. 本地启动 WorldMonitor。打开 `/dashboard` 检查含二十一个工具的仪表板；不要使用 `/embed`。若要检查含两个工具的静态首页，请先运行 `npm run build:pro`，再打开 `/pro/welcome.html`。本地 Vite 的 `/` 会加载仪表板 SPA，只有生产环境才把 `/` 重写到欢迎页。
...
`openCountryBrief`、`switch_monitor`、`set_panel_enabled`、`set_map_layers`、`select_dashboard_tab`、`create_dashboard_tab`、`rename_dashboard_tab` 与 `delete_dashboard_tab` 会继续被拦截并返回不可用。`openCountryBrief` 可能启动服务器端 LLM 简报生成（`premiumFetch` → `/api/intelligence/v1/get-country-intel-brief` → `callLlm`）；即使浏览器随后取消调用方的 Promise，这次生成仍会消耗调用者的每日额度。`switch_monitor` 可能持久化桌面变体并重新加载，或把当前标签导航到其他源。`set_panel_enabled` 和 `set_map_layers` 会把仪表板状态持久化到浏览器存储，启用 AIS 图层时还会建立网络连接。仪表板标签页变更会持久化 `worldmonitor-tabs-v1` 与实时面板工作区。`open_search_result` 是选择器：无标签深链的已启用面板等视图状态结果可以执行；会持久化、消耗配额或离开当前页面的结果仍被拒绝。`panel:consumer-prices@world` 这类标签深链会写入面板设置，因此在没有目标侧信号时保持拦截。这些影响可能在取消后继续存在，因此只有浏览器提供目标侧信号时才可用。只读的 `list_map_layers` 和 `list_dashboard_tabs` 目录仍可调用。
...
六个仪表板变体都注册相同的二十一个命令式工具。登录和权益变化不会改变注册集合；每次调用都会重新检查实时状态。

在没有提供目标侧调用信号的浏览器上，可逆的视图状态工具仍会执行：由于此类浏览器无法取消它们，你中止的调用仍会执行完毕，其可见的仪表板变化也会生效。而 `openCountryBrief`、`switch_monitor`、`set_panel_enabled`、`set_map_layers`、`select_dashboard_tab`、`create_dashboard_tab`、`rename_dashboard_tab` 与 `delete_dashboard_tab` 会返回不可用。`open_search_result` 仍会执行视图状态结果（例如打开已启用的实时摄像头面板），并拒绝持久化、消耗配额或离开当前页面的结果，包括会写入设置的面板标签深链。国家简报工具可能在调用方取消后继续消耗每日 LLM 额度，监视器切换可能持久化并重新加载或导航到其他源，面板、图层和标签页变更会在本次会话之外持久化。只读的 `list_map_layers` 和 `list_dashboard_tabs` 目录仍会执行。参见[可用性](#availability)。
...
| `open_dashboard_panel` | 必填字符串 `panelId`，长度 1–96，模式 `^[A-Za-z0-9][A-Za-z0-9@_-]*$`；不允许其他属性。 | 经权益感知 UI 路径打开并滚动到当前已启用的可用面板。已禁用面板返回 `panel_disabled`；使用 `set_panel_enabled` 更改目录面板是否启用。此工具不会自行启用面板。 |
...
| 搜索采购机会 | `list_dashboard_panels` → 如需启用全球采购面板则调用 `set_panel_enabled`，或请用户启用它 → 发现 `search_procurement` → 调用 | `open_dashboard_panel` 不能启用已禁用的面板。只有当有权益的表单已连接、可见、数据就绪且空闲时，该声明式工具才存在。工具消失表示状态变化，并非注册失败。 |
...
| 调用返回 `target_cancellation_unsupported` | 浏览器接受了 WebMCP，但没有把调用的 `AbortSignal` 交给页面 | 使用只读工具或可逆视图状态工具。不得绕过 `openCountryBrief`、`switch_monitor`、`set_panel_enabled`、`set_map_layers` 或仪表板标签页变更的拒绝。对于 `open_search_result`，请选择视图状态结果，或等待能够取消持久化工作的宿主。 |
...
| `src/config/panel-enablement.ts` 和 `src/app/panel-enablement.ts` | `set_panel_enabled` 使用的面板启用策略，以及现有设置持久化/应用路径。 |
