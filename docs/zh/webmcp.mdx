...
即便如此，WorldMonitor 仍会在这些版本上执行会修改仪表板的工具。每个此类工具只会改变用户自己屏幕上可见的仪表板视图状态——不做导航、不写入数据、无外部副作用——因此一次无法取消的调用，最坏结果也只是一处用户可以自行撤销的可见变化。若改为拒绝执行，则会在迄今发布的所有浏览器上停用全部会修改仪表板的工具、只剩下那一对只读工具，而换来的仅仅是避免地图被移动。凡是可能产生用户无法一眼看到并自行撤销的效果的工具，仍会被继续拦截，并在浏览器提供目标侧信号之前返回不可用。
...
在没有提供目标侧调用信号的浏览器上，这八个工具同样都会执行。由于此类浏览器无法取消它们，你中止的调用仍会执行完毕，其可见的仪表板变化也会生效——参见[可用性](#availability)。凡是用户无法自行撤销其效果的工具则会被拦截并返回不可用；目前这八个工具中没有任何一个被拦截。
