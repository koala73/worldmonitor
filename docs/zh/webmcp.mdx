...
**仅在普通 ChatGPT 对话中提到网站，并不会让 ChatGPT 发现 WebMCP 工具。** 普通 ChatGPT 对话（包括 iOS 或 Android 应用中的对话）并不是拥有 `document.modelContext` 的 Chrome 标签页。要从 ChatGPT 使用 WorldMonitor，请先在 ChatGPT 开发者模式中将 `https://worldmonitor.app/mcp` 添加为自定义应用，然后在新对话中启用该应用。请参阅 [ChatGPT 设置](/zh/mcp-overview#chatgpt)。
</Warning>
