...
3. 完全重新启动 Chrome。
4. 本地启动 WorldMonitor。打开 `/dashboard` 检查仪表板 SPA 工具清单；不要使用 `/embed`。若要检查含两个工具的静态首页，请先运行 `npm run build:pro`，再打开 `/pro/welcome.html`。本地 Vite 的 `/` 会加载仪表板 SPA，只有生产环境才把 `/` 重写到欢迎页。
...
六个仪表板变体都注册同一套命令式 SPA 工具清单。登录和权益变化不会改变注册集合；每次调用都会重新检查实时状态。
...
| `src/App.ts` 和 `src/app/webmcp-dashboard.ts` 和 `src/app/event-handlers.ts` | 启动顺序、UI 就绪门禁、销毁、仪表板上下文、监视器/设置/提醒导航，以及人工路径操作绑定。 |
