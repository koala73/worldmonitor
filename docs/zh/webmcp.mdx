...
Chrome 149–151 虽已暴露 `registerTool()`，但调用已注册回调时只传入 input，并非文档所述的 `execute(input, { signal })` 形式。在这些版本中，取消在两个方向上都是无效的：`executeTool()` 会忽略其可选参数中的 `signal`，因此 abort 既不会拒绝调用方的 Promise，也不会中止页面中已在运行的工作。

即便如此，WorldMonitor 仍会在这些版本上执行那些只改变可见视图状态的工具——不做导航、不写入数据、无外部副作用——因为此时一次无法取消的调用，最坏结果也只是一处用户可以自行撤销的可见变化。`openCountryBrief`、`openSearch`、`open_dashboard_panel` 与 `set_map_view` 均可正常使用。
...
该协作式目标侧取消证明要求浏览器把调用信号传给已注册回调。若浏览器仍使用单参数回调（迄今发布的所有 Chrome 皆是如此），上面的代码片段永远不会进入 `AbortError` 分支：abort 会被忽略，调用方的 Promise 将以工具的正常结果 resolve，页面内的工作也会照常完成。在这些版本上，应将取消视为不可用，而非尽力而为。
