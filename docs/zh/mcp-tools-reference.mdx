...
与仪表板共享的预计算、有引用依据的世界情报简报，读取 `news:insights:v1` 快照。MCP 调用不会运行单次 LLM。`geo_context` 为兼容旧客户端而保留，但不会缩小或重新生成全局快照。

**参数：**

| 名称 | 类型 | 必填 | 描述 |
|------|------|---------:|-------------|
| `geo_context` | string | 否 | 旧客户端兼容输入；对这个全局预计算快照会被忽略，不会聚焦或重新生成简报 |

- **来源：** 与仪表板 bootstrap 水合共享的规范种子 Redis key `news:insights:v1`。
- **类型：** 缓存支持的 RPC — 返回生产者生成的引用锁定简报、故事行和来源记录；快照缺失或格式错误时会安全失败，不会回退到未受门控的实时摘要。
- **新鲜度：** 由 insights seeder 的运行节奏限制；简报同时返回 `generatedAt` 和 `status`。
