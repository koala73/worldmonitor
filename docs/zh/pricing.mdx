...
- **Free** — 58 个地图图层（除 Resilience 图层属于 Pro 外全部免费）、500+ 精选数据源、国家简报、热点、不稳定性评分、咽喉要道、级联分析、突发警报和监视列表，共 3 个仪表盘标签页。仪表盘的刷新频率通常为 5–15 分钟。
- **Pro** — 涵盖 Free 的全部内容，外加跨 30+ 实时服务并带引用的 WM Analyst 聊天、Scenario Engine、Route Explorer（路线探索器）、个人 AI 摘要（每日 / 每日两次 / 每周，通过 Slack、Discord、Telegram、邮件、webhook 发送）、自定义小部件构建器（10 个自定义仪表盘）、Resilience 地图图层、每天 500 次仪表盘 AI 请求，以及单一密钥下的 MCP 访问（50 次调用/天）。个人许可。
