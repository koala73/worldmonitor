...
| 决策关键国家超出预算、时间戳位于未来、缺少可用时间戳，或本次运行中完全缺失 | `STALE_CONTENT` |
...
告警将只是在测量轮换而非事故。全局范围的内容过时在设计上属于常态：seeder 每次
...
完全相同的固定范围、预算与激活标记，因此 MCP 消费者不可能对本端点判定为
`STALE_CONTENT` 的键读到 `stale: false`。两个面在读取时都会重新计算观测年龄，并共用
同一个评估器（`api/_content-freshness.js`），且该一致性是逐字段断言的，而非仅靠注释
声明。两者读取激活标记时都使用 `EXISTS`，因此其存储值不可能让两个面产生分歧。MCP
还会把*无法读取*的标记视为未知而非缺失 —— 只有在标记被成功读取且确认缺失时才授予
放宽，因此一次 Redis 抖动无法悄悄延长一个已经结束的上线窗口。MCP 调用方看到的仍是
单一布尔值：`stale` 不会说明是*哪个*维度失败，因此 `/api/health` 仍是唯一会指名过时
国家的面。
