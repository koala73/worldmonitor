...
| `RELAY_GATE_MISCONFIGURED` | 严重 | 提供 `/api/health` 的生产构建看不到 `CONVEX_SITE_URL` 或 `CONVEX_TENANT_RELAY_SECRET`，因此结账、客户门户和通知渠道在到达 Convex 之前就返回 503。该状态出现在 `relayGatewayGate` 存活性条目上；它不是数据键，也不计入 `summary.total`。设置变量后必须推送一个新的提交：同一提交的重新部署会被忽略构建步骤取消 |
...
| `RELAY_GATE_UNREACHABLE` | 警告 | 网关探测没有得到可分类的回答（网络错误、超时、5xx，或除 `401` / `400 MISSING_FIELDS` 之外的任何响应）。这是传输层判定，不是凭据判定；连续多次扫描持续出现意味着 Convex 故障或 `CONVEX_SITE_URL` 错误 |
