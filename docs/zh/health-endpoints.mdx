...
| `RELAY_GATE_UNREACHABLE` | 宽限期后为警告 | 网关探测没有得到可分类的回答（网络错误、超时、5xx，或除 `401` / `400 MISSING_FIELDS` 之外的任何响应）。这是传输层判定，不是凭据判定。首次出现时带有三分钟的 `transportGraceUntil`：在此期间计入 `ok`、出现在紧凑响应的 `pending` 中，且不是运维问题；截止时间过后仍然存在的故障会移入 `problems` 并计入 `summary.warn`。连续多次扫描持续出现意味着 Convex 故障或 `CONVEX_SITE_URL` 错误 |
