...
商品脆弱性批次不采用“非空即健康”。所有脆弱性 RPC 实际读取的权威批次指针必须存在。国家投影必须达到配置的国家下限，每个已覆盖国家必须包含全部已审核映射行，并且本次运行必须实际观测到完整的商品、HS4 和运输 HS2 输入集合；注册表常量本身不能满足实测覆盖检查。反向投影必须达到配置的咽喉要道下限。任何不足都会报告 `COVERAGE_PARTIAL`。首个原子批次激活前，两个检查保持 rollout pending。
