...
连接需要登录：服务器会在未认证客户端的第一个请求上返回质询，这正是让你的 MCP 客户端弹出登录窗口的信号。免费的 WorldMonitor 账户即可完成连接；登录后 `get_sources` 不产生任何费用（无需订阅，也不消耗每日配额），少量免费额度可用于缓存类工具。其他所有数据工具都需要订阅。要使用这些工具，你需要以下之一：
...
- **使用 API key 时，通过请求头传递。** 请使用单参数形式 `X-WorldMonitor-Key:wm_…`，**冒号后不要有空格**——这可以避开客户端/`npx` 对 `args` 中空格的转义问题（`mcp-remote` 已针对 Windows 上的 Cursor 与 Claude Desktop 记录该问题）。携带 key 的请求不会被质询。
- **偏好 OAuth？** 省略 `--header`。服务器会对 `mcp-remote` 的第一个请求返回 `401` 质询，从而在连接时就打开其浏览器授权流程——这是 `mcp-remote` 处理得最可靠的路径（动态客户端注册在 `worldmonitor.app` 与 `www.worldmonitor.app` 上均可用）。免费账户即可完成该流程；订阅工具则取决于你的套餐。
