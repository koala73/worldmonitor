...
- **使用 API key 时，通过请求头传递。** 携带 key 的请求不会被质询。请使用单参数形式 `X-WorldMonitor-Key:wm_…`，**冒号后不要有空格**——这可以避开客户端/`npx` 对 `args` 中空格的转义问题（`mcp-remote` 已针对 Windows 上的 Cursor 与 Claude Desktop 记录该问题）。
