...
完整的自托管栈（仪表盘 + Railway 式中继 + Redis + 种子脚本）需要 **Node.js 22+**、Docker 或 Podman，以及没有安全默认值的密钥 — `RELAY_SHARED_SECRET`、`REDIS_PASSWORD` 和 `REDIS_TOKEN` 必须在 `docker compose up` 之前设置。完整的操作步骤请参见仓库中的 `SELF_HOSTING.md` 指南。 关于自托管包含什么、不包含什么以及许可立场，请参见[自托管](/zh/self-hosting)。
