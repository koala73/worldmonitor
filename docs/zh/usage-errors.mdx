...
| `entitlement_verification_unavailable` | 无法连接计费后端以校验访问权限 —— 在所有已认证入口均可能出现（`wm_` API 密钥、bootstrap、会话/bearer 层级检查、MCP）。可重试的 503，携带 `Retry-After: 5` 与 `X-Billing-Verification: entitlement_verification_unavailable`。 |
