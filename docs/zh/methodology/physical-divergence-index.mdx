...
每个点和响应都声明 `methodologyVersion: physical-divergence-v1`。
...
系统分别计算绝对状态和相对状态，并选择较高者。相对阈值只适用于正溢价。
...
单金属压力指数范围为 0 到 100。绝对幅度在压缩后的分段顶点 45（elevated）、70（stressed）、90（接近 extreme）之间插值；只有突破绝对 extreme 溢价阈值（黄金 5%、白银 20%）才发布 `100`。
...
| `physical-divergence-v1` | 2026-08-30 | 指数下限压缩为 0 / 45 / 70 / 90；绝对分段顶点嵌在这些下限之下；`100` 留给绝对 extreme，避免仅相对 extreme 顶满刻度（#7423）。 |
