---
title: "国家韧性指数"
description: "196 国可排名范围的实时韧性评分，覆盖 6 个领域和 20 个活跃维度，将结构基线指标与实时压力信号结合为每 6 小时更新的 0-100 韧性分数。以 OECD/JRC 方法论对等标准发布，包含透明的目标阈值、覆盖率追踪和正式的四类插补分类法。"
---

WorldMonitor 国家韧性指数（CRI）以 0-100 分对 196 国可排名范围进行评分，将长期结构能力与当前运营压力结合为可操作的韧性指标。上游静态/来源源可以包含更广泛的领土集合，但公开的 CRI 排名在分数发布前过滤到可排名范围。CRI 不依赖静态国家风险评级，而是每 6 小时从官方和权威来源更新，并公开完整的来源溯源、覆盖率和插补上下文，让分析师能准确看到分数*为什么*变动以及其中多少是真实数据与插补数据。
...
对于实时部署状态，请使用[运行时清单](/api/resilience/v1/get-runtime-manifest)。它报告活动公式标签、静态 `dataVersion`、排名缓存元数据、安全派生构造版本如 `constructVersions.energy`（`legacy` 或 `v2`），以及公共 `scoreInterval`/`rankStable` 支持路径的 `intervals` 可用性元数据，而不暴露部署标识符、原始 `RESILIENCE_*` 标志状态或内部缓存键。参考版仍是冻结的已发布包；不应将其视为当前生产中活动运行时状态的证据。
...
3. **遗留 `scoreEnergy` 路径无核能信用。** v2 构造通过将 `renewShare` + 新核能份额 + 水电合并为单一 `lowCarbonGenerationShare` 指标来信贷稳定的低碳发电，来源于世界银行 `EG.ELC.NUCL.ZS + EG.ELC.RNEW.ZS + EG.ELC.HYRO.ZS`。水电显式求和因为 WB RNEW 不含水电；没有 HYRO，水电大国（挪威约 95%、巴拉圭约 99%、巴西约 65%、加拿大约 60%）在此 0.20 权重信号上将接近零分，尽管拥有接近 100% 的低碳电网。
...
WorldMonitor 国家韧性指数以 0-100 分对 196 国可排名范围评分，覆盖 6 个领域和 20 个活跃维度（加上 2 个结构性退役维度为模式连续性在注册表中以 `coverage=0` 保留）。排名处理器仍可将低置信度或标题不合格的国家路由到 `greyedOut[]`，但可排名范围本身由提交的 UN 成员 + SAR 白名单固定。它将结构基线指标（治理质量、健康基础设施、财政能力）与实时压力信号（网络威胁、冲突事件、航运中断）和恢复能力指标（财政空间、储备、进口集中度）结合，产出每 6 小时更新的单一韧性分数。
...
PR 3 §3.5 第 2 点退役了 BIS 支持的核心构造。BIS REER 和 DSR 仅覆盖 64 个 BIS 报告经济体，因此旧复合指标对约三分之二的 196 国可排名范围落入 curated_list_absent（覆盖率 0.3）或薄弱的 IMF 代理（覆盖率 0.45）。重建的维度使用两个全球覆盖的世界银行/IMF 系列。
...
OFAC `sanctionCount` 组件（原权重 0.45）被删除 — 统计指定方注册地是公司金融责任指标，不是国家韧性指标（像阿联酋或新加坡这样的中转枢纽承载许多空壳公司条目，但不反映所在国的结构韧性）。剩余 3 个组件重新加权至总计 1.0。独立的 `financialSystemExposure` 维度以 BIS Locational Banking Statistics + WB IDS 短期外债 + FATF AML/CFT 列表状态的结构性制裁暴露替代了被删除的代理。

关于完整构造理由和被拒绝的替代方案（项目权重分类、中转枢纽排除列表），请参见 [known-limitations.md § tradeSanctions → tradePolicy](./known-limitations.md#tradesanctions--tradepolicy-ofac-domicile-component-dropped-ship-1-2026-04-25)。
...
该维度使用**失败关闭预检**模式（镜像 `scoreEnergy` v2）：所有 3 个必需的种子信封（`economic:wb-external-debt:v1`、`economic:bis-lbs:v1`、`economic:fatf-listing:v1`）必须可达。缺失 seed-meta 表明 Railway bundle 中断，表现为 `imputationClass='source-failure'` 而非静默将维度归零。按国家的数据间隙是不同的：按组件读取返回 null，槽位从加权混合中排除。

| 指标 | 描述 | 方向 | 目标阈值（最差-最好） | 权重 | 来源 | 频率 |
|---|---|---|---|---|---|---|
| shortTermExternalDebtPctGni | 短期外债占 GNI 百分比（WB IDS DT.DOD.DSTC.IR.ZS × DT.DOD.DECT.GN.ZS）；IMF Article IV 脆弱性阈值为 GNI 的 15% | 越低越好 | 15 - 0 | 0.35 | 世界银行 IDS | 年度 |
| bisLbsXborderPctGdp | BIS LBS 按母国跨境债权总和（美国/英国/主要欧盟/瑞士/日本/加拿大/澳大利亚/新加坡）占 GDP 百分比；U 形区间 — 隔离（低于 5%）和过度暴露（高于 60%）均低分 | 越低越好（U 形） | 60 - 25 | 0.30 | BIS LBS | 季度 |
| fatfListingStatus | FATF AML/CFT 列表状态 — 黑名单（呼吁行动）→ 0，灰名单（加强监控）→ 30，合规 → 100 | 越高越好 | 0 - 100 | 0.20 | FATF | 月度 |
| financialCenterRedundancy | 拥有非平凡（>1% GDP）跨境债权的不同 BIS LBS 按母国报告者计数；奖励多对手方金融中心，平衡组件 2 过度暴露惩罚 | 越高越好 | 1 - 10 | 0.15 | BIS LBS | 季度 |

**覆盖率**：WB IDS 仅发布约 125 个 LMIC；HIC 落入 BIS LBS 结构性暴露组件（覆盖约 200 国）。FATF + BIS LBS 一起覆盖几乎所有显式国家。
...
关于完整构造理由、考虑的替代方案（项目权重分类、中转枢纽排除、单维公式重写、完全删除）和分阶段推出决策，请参见 [financial-system-exposure.md](./financial-system-exposure.md)。
...
| 指标 | 描述 | 方向 | 目标阈值（最差-最好） | 权重 | 来源 | 频率 |
|---|---|---|---|---|---|---|
| cyberThreats | 严重性加权网络威胁计数（关键 3x、高 2x、中 1x、低 0.5x） | 越低越好 | 25 - 0 | 0.45 | 网络威胁源 | 每日 |
...
| lowCarbonGenerationShare | 核能 + 可再生（不含水电）+ 水电占发电份额（`EG.ELC.NUCL.ZS + EG.ELC.RNEW.ZS + EG.ELC.HYRO.ZS`）。水电单独求和因为 WB RNEW 不含水电。 | 越高越好 | 0 - 80 | 0.20 | 世界银行 | 年度 |
...
| 指标 | 描述 | 方向 | 目标阈值（最差-最好） | 权重 | 来源 | 频率 |
|---|---|---|---|---|---|---|
| ucdpConflict | UCDP 武装冲突：事件数*2 + 类型权重 + sqrt(死亡数)，按百万人口归一化，0.5M 下限 | 越低越好 | 15 - 0 | 0.65 | UCDP | 实时 |
...
| recoveryConflictPressure | UCDP 冲突指标反转为国家连续性 | 越低越好 | 30 - 0 | 0.30 | UCDP | 实时 |
...
| `unmonitored` | 来源是精选列表，可能不覆盖每个国家。缺失是模糊的；保守惩罚。 | 50 至 60 | 0.3 至 0.4 | BIS 汇率和信贷、WTO 贸易数据、OECD ICU 容量 |
...
通用插补条目在 `IMPUTE` 表中声明并跨维度共享。按指标覆盖在 `IMPUTE` 表中有自己的分数和确定性值，并继承或覆盖类标签。每个条目在 `tests/resilience-dimension-scorers.test.mts` 中回归测试以防止静默偏移。
...
| `bisEer` 和 `bisCredit` | `unmonitored` | 50 | 0.3 | 对 `curated_list_absent` 的共享引用；相同标签 |
...
总覆盖率低于 **0.40** 的国家在 UI 中灰显并从排名中排除。其分数数据太稀疏，无意义。
...
| 世界银行（WDI）| 电力获取、铺设道路、储备、关税、电力消费 | 年度 | 全球 |
| 世界银行（WGI）| 6 治理指标 | 年度 | 全球 |
| BIS | 实际有效汇率 | 月度 | 约 60 国 |
...
| 键 | 类型 | TTL | 写入方 | 读取方 |
|---|---|---|---|---|
| `resilience:score:v24:{countryCode}` | JSON | 6 小时 | `server/worldmonitor/resilience/v1/_shared.ts` 中的 `buildResilienceScore` | `getResilienceScore` 处理器 |
| `resilience:ranking:v24` | JSON | 12 小时 | `getResilienceRanking` 暖路径，仅当至少 90% 国家已评分时（`RANKING_CACHE_MIN_COVERAGE = 0.90`）| `getResilienceRanking` 处理器 |
| `resilience:history:v19:{countryCode}` | 有序集合 | 无限期，修剪至 30 天 | 评分期间 `appendHistory` | 趋势和 `change30d` 计算 |
| `resilience:intervals:v8:{countryCode}` | JSON | 7 天；通过 `seed-meta`/健康频率监控新鲜度 | `scripts/seed-resilience-scores.mjs` | `getResilienceScore`（可选 `scoreInterval` 字段）和 `getResilienceRanking`（`rankStable`）|
...
公共运行时清单有意不回显这些 Redis 键名。其 `intervals` 对象报告使用固定公共样本国家、当前区间方法论标签和最新安全观测时间戳的派生可用性检查。如果区间数据缺失、对活动公式过期或由不同方法论产出，`intervals.available` 为 `false` 而非抛出；这是用户面 `scoreInterval` 和 `rankStable` 支持当前未由可读区间数据支撑的公共审计信号。

### dataVersion 语义

每个 `GetResilienceScoreResponse` 上的 `dataVersion` 字段是 `seed-meta:resilience:static` 中存储的 `fetchedAt` 时间戳的 ISO 日期。它反映 Railway 静态种子作业的最近成功运行；widget 在页脚渲染为 `Seed date YYYY-MM-DD`。标签比"Data"更窄，因为实时输入（冲突事件、中断、价格）可在静态 bundle 运行后按自身频率刷新 — 按维度新鲜度通过置信度网格中的新鲜度徽章单独呈现。
...
3. 读取国家切片的实时信号键（UCDP、UNHCR、中断、网络威胁、价格、航运压力等）。
...
1. **来源全面性标志（PR #3452，§U5）。** 19 个指标标记 `comprehensive: false`，因为其缺失不暗示"没有发生" — 仅事件源（UCDP、IPC、GPS 干扰、互联网中断）、部分覆盖精选列表（BIS-64、WTO top-50、FATF）和仅双边系列。对这些，`IMPUTE` 从乐观的 `stable-absence`（85，确定性 0.6）切换到保守的 `unmonitored`（50，确定性 0.3）。之前借助"缺失即好消息"假设的微型国家不再获得免费提升。
...
**缓存前缀升级。** `resilience:score:v15:` → `:v16:` → `:v17:` → `:v18:` → `:v19:` → `:v20:` → `:v21:` → `:v22:` → `:v23:` → `:v24:`；`resilience:ranking:v15` → `v16` → `v17` → `v18` → `v19` → `v20` → `v21` → `v22` → `v23` → `v24`；`resilience:history:v10:` → `:v11:` → `:v12:` → `:v13:` → `:v14:` → `:v15:` → `:v16:` → `:v17:` → `:v18:` → `:v19:`。v17 升级随标题合格门（PR #3469）发布，因为 `headlineEligible` 成为必需字段；缓存的 v16 条目省略它，v17 的保守防御默认值为 `headlineEligible: false`（异常缺失 → 降级）以匹配 v17 的"每个合法写入器标记字段"合约。v18 升级随 §U8.1（净进口分母扩展到 `liquidReserveAdequacy`）发布，v19 随 cyberDigital 按快照上限，v20 为陈旧降级推出保留，v21 发布在活跃 `pc` 公式内应用领域设计权重的 P1-1 支柱聚合修复，v22 发布第二轮通胀稳定性和 NaN 安全混合修复，v23 发布进口 HHI 陈旧/缺失来源年确定性降级、P3-8 中断源观测安静语义和 WTO 贸易政策严重性评分器修复（全部批量到同代），v24 发布第五轮 R5-2 / PR #4101 治理 WGI 指标槽语义修复。区间缓存也轮换到 `resilience:intervals:v8:` 使旧敏感度带不与 v24 分数一起提供。
...
**v17.2 — `cyberDigital` 按快照爆发上限（#3971）。** 评分器在应用现有 `normalizeLowerBetter(weightedCount, 0, 25)` 变换前将每个国家的总严重性加权网络威胁计数按快照上限为 8，因此 `cyber:threats:v2` 中的同日飙升不再能将网络子组件饱和到 0 并摆动国家 5+ 排名位置。这是故意的**按快照**上限，**非**多日平滑：实时源在约获取时标记 `lastSeenAt` 且从不填充 `firstSeenAt`（2026-06-01 验证实时：958 条记录中 0 条携带非零 `firstSeenAt`，每个国家的 `lastSeenAt` 解析为单日），因此源不暴露供评分器平均的跨日展开。区分瞬态爆发和持续多日压力需要跨快照状态（见下方 `cyberDigital` 警告）且此处不声称。修复随缓存前缀升级（`resilience:score:` 和 `resilience:ranking:` 从 `v18` → `v19`，`resilience:history:` 从 `v13` → `v14`）发布，使缓存未上限分数和 30 天趋势点不与上限分数混合。
...
**v17.6 — 中断源观测安静语义（审计 P3-8）。** `infrastructure` 现在将带空 `outages[]` 数组的加载中断源视为观测安静（分数 100），匹配 `cyberDigital` 对相同上游源的无事件语义。之前的基础设施评分器在惩罚为零时删除中断组件，因此无当前中断事件的国家失去 0.25 组件而非因观测缺失获得信用。这在与上方进口 HHI 降级和中断语义相同的 `v23` 分数/`v23` 排名/`v18` 历史/`v7` 区间缓存代中发布 — 两者都是覆盖率影响和分数影响的新鲜度修复，批量到 v23，使已发布分数、排名聚合、30 天趋势和敏感度带从相同评分器数学重新计算。
...
- **`cyberDigital` 无跨日平滑。** 同快照爆发波动受上方按快照严重性加权上限限制，种子器 TTL/最后良好行为保护免受空或失败获取 — 但两者都不是多日平滑。因为 `cyber:threats:v2` 在约获取时标记 `lastSeenAt` 且不填充 `firstSeenAt`，评分器无法区分一日飙升和持续多日压力：两者都呈现为单一上限快照。真正的爆发对持续区分需要 (a) 网络种子器修复以填充真实发现时间戳（`firstSeenAt`）使评分器可按发现日分组，或 (b) 韧性种子器维护的跨快照滚动/EWMA 状态。两者都作为后续追踪；在此之前维度是有界时间点信号，非平滑信号。
...
| **时效性** | 6.5 | 结构来源为年度（WGI、GPI、RSF、WHO、IMF 宏观）并主导指数总权重。BIS EER 为月度。新鲜度分类器（T1.5）在维度级呈现，使用户可看到国家分数的哪些部分已 12 个月。十二个压力侧指标已通过跨源栈以实时/小时或日频率运行（`ucdpConflict`、`internetOutages`、`infraOutages`、`unrestEvents` 实时；`socialVelocity` 通过小时 Reddit relay 带 180 分钟健康预算；`cyberThreats`、`gpsJamming`、`shippingStress`、`transitDisruption`、`euGasStorageStress`、`energyPriceStress`、`newsThreatScore` 每日）。间隙：实时冲击支柱依赖这些信号但结构支柱仍受年度来源限制；Phase 2 T2.2 添加日频率 FX 波动率以缩小货币外部维度频率差距，Phase 3 参考版拆分将按支柱形式化年度与滚动频率。 |
...
- **T2.2b**（#2987）：恢复能力支柱含新 `recovery` 领域的 6 个新维度：财政空间（偿债比率）、储备充足性（进口月数）、短期外债覆盖、进口集中度（HHI）、医院激增容量和国家连续性复合（WGI 子集）。五个新种子器遵循 Railway 金标准模式（3 个真实数据源，2 个待源配置存根）。缓存键升级到当前版本。
...
- **T2.9**（#2992）：informationCognitive 维度的语言和来源密度归一化。RSF 新闻自由和社交速度分数按来源集的语言覆盖加权，以纠正英语媒体偏差。维度在归一化后提升回 Core 层。
...
**敏感度和比较工件**（2026-04-21，提交 `048bb8b`，52 国样本，在比较脚本修正为使用生产 `buildPillarList` 聚合后重新生成）：[`docs/snapshots/resilience-pillar-sensitivity-2026-04-21.json`](../snapshots/resilience-pillar-sensitivity-2026-04-21.json)。
...
1. **特性标志**：`RESILIENCE_PILLAR_COMBINE_ENABLED`，每次调用从 `process.env` 动态读取。生产 Vercel 和 Railway 验证环境设为 `true`；未设置/`false` 仍为本地默认和回退路径。
...
回退：设 `RESILIENCE_PILLAR_COMBINE_ENABLED=false`，刷新当前 `resilience:score:v24:*`、`resilience:ranking:v24` 和 `resilience:history:v19:*` 键（或等待 TTL 过期）。6 领域公式与支柱组合在 `_shared.ts` 中并存，回退无需代码变更。
...
| **时效性** | 7.0 | 13 个压力侧指标以实时/日频率。语言归一化纠正来源密度偏差。恢复能力添加月度储备 + 债务信号。间隙：结构来源仍为年度（WGI/GPI/RSF/WHO）。Phase 3 参考版拆分按支柱形式化年度与滚动频率。 |
...
**状态：生产中活动。** 韧性修复计划中的 PR 1（`docs/plans/2026-04-22-001-fix-resilience-scorer-structural-bias-plan.md`）。在一个连贯 PR 中解决构造错误 §3.1、§3.2、§3.3。最初在 `RESILIENCE_ENERGY_V2_ENABLED` 后落地；生产现在标志开启运行，运行时清单报告 `constructVersions.energy="v2"`，遗留评分器保留为标志关闭回退路径。
...
**状态：落地中。** 韧性修复计划中的 PR 3（`docs/plans/2026-04-22-001-fix-resilience-scorer-structural-bias-plan.md`）。解决计划 §3.5（核心分数中的死信号和仅区域信号）和 §3.6（基于覆盖率的标称权重上限）。与 PR 1 不同，无标志 — 变更立即应用，因为退役构造从未产生全球信号。
...
- **§3.5 第 2 点 — `currencyExternal` 在 IMF 通胀 + WB 储备上重建。** BIS REER / DSR 仅覆盖 64 个 BIS 报告经济体；旧复合指标对约三分之二的 196 国可排名范围落入 curated_list_absent（覆盖率 0.3）或薄弱 IMF 代理（覆盖率 0.45）。新维度：`inflationStability`（IMF WEO 标题通胀，权重 0.60）+ `fxReservesAdequacy`（WB 储备折合月数，权重 0.40）。覆盖阶梯：两者=0.85、仅通胀=0.55、仅储备=0.40、均无=0.30。遗留 `fxVolatility` + `fxDeviation` 作为 `tier='experimental'` 保留在 64 个 BIS 经济体的国家下钻中。
