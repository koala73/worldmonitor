---
title: "国家韧性指数"
description: "面向 196 个国家的实时韧性评分体系，覆盖 6 大领域与 21 个活跃维度 —— 一个每 6 小时刷新一次的 0-100 综合指数，采用 OECD、JRC 等国际对等标准进行同侪基准比较，用于衡量国家治理、经济、社会与安全韧性的相对强弱、动态变化与结构性差距。"
---
...
WorldMonitor 国家韧性指数以 0-100 分对 196 国公开可排名范围评分，覆盖 6 个领域和 21 个活跃维度（加上 2 个结构性退役维度为模式连续性在注册表中以 `coverage=0` 保留）。排名处理器仍可将低置信度或标题不合格的国家路由到 `greyedOut[]`，但可排名范围本身由提交的 UN 成员 + SAR 白名单固定。它将结构基线指标（治理质量、健康基础设施、财政能力）与实时压力信号（网络威胁、冲突事件、航运中断）和恢复能力指标（财政空间、储备、进口集中度）结合，产出每 6 小时更新的单一韧性分数。
...
| 键 | 类型 | TTL | 写入方 | 读取方 |
|---|---|---|---|---|
| `resilience:score:v27:{countryCode}` | JSON | 6 小时 | `server/worldmonitor/resilience/v1/_shared.ts` 中的 `buildResilienceScore` | `getResilienceScore` 处理器 |
| `resilience:ranking:v27` | JSON | 12 小时 | `getResilienceRanking` 暖路径，仅当至少 90% 国家已评分时（`RANKING_CACHE_MIN_COVERAGE = 0.90`） | `getResilienceRanking` 处理器 |
| `resilience:history:v21:{countryCode}` | 有序集合 | 无限期，修剪至 30 天 | 评分期间 `appendHistory` | 趋势和 `change30d` 计算 |
| `resilience:intervals:v10:{countryCode}` | JSON | 7 天；通过 `seed-meta`/健康频率监控新鲜度 | `scripts/seed-resilience-scores.mjs` | `getResilienceScore`（可选 `scoreInterval` 字段）和 `getResilienceRanking`（`rankStable`） |
...
回退：设 `RESILIENCE_PILLAR_COMBINE_ENABLED=false`，刷新当前 `resilience:score:v27:*`、`resilience:ranking:v27` 和 `resilience:history:v21:*` 键（或等待 TTL 过期）。6 领域公式与支柱组合在 `_shared.ts` 中并存，回退无需代码变更。
