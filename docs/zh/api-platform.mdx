...
桌面端只发布一个二进制文件,所有变体均在安装后于应用内切换,因此每个受支持的值都会解析到所请求平台的同一个 World Monitor 资产 —— 该参数只记录调用方来自哪个变体,并不会选择不同的下载。无法识别的值将重定向到 [releases/latest](https://github.com/koala73/worldmonitor/releases/latest),且不会调用 GitHub。省略 `variant` 时返回匹配该平台的第一个资产。
