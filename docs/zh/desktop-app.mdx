...
桌面应用由 World Monitor 源代码构建，**其二进制文件依据 AGPL-3.0-only 授权给你** —— 你可以按 AGPL 的条款运行、研究、修改和再分发它。源代码：[github.com/koala73/worldmonitor](https://github.com/koala73/worldmonitor)。每个打包版本都会在其 resources 目录中附带构建时生成的 `THIRD-PARTY-NOTICES.md`，其中收录了它所包含的 npm 包与 Rust crate 的原文声明；随附的 Node.js 运行时在同一目录中携带自己的 `LICENSE`。
