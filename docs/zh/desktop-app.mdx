...
- **OS 钥匙串集成** —— API 密钥存储在系统凭据管理器中（macOS Keychain、Windows Credential Manager、Linux Secret Service），绝不以明文文件形式存储。 Linux 通过同一个 `keyring` crate 使用 Secret Service API；没有可激活的 Secret Service 提供者的 Linux 桌面目前会以空保险库启动（#802）。
...
API 密钥存储在操作系统的凭据管理器中（macOS Keychain、Windows Credential Manager、Linux Secret Service）—— 绝不以明文配置文件形式存储。 Linux 通过同一个 `keyring` crate 使用 Secret Service API；没有可激活的 Secret Service 提供者的 Linux 桌面目前会以空保险库启动（#802）。所有密钥都合并到钥匙串中的单个 JSON 保险库条目里，因此无论配置了多少密钥，应用启动都只需要一次 OS 授权提示。
...
桌面应用通过轮询 `api.worldmonitor.app/api/version` 检查新版本 —— 启动时一次（延迟 5 秒），之后每 6 小时一次。当检测到较新版本时（semver 比较），会出现一个非侵入式的更新徽章，附带指向 GitHub Release 页面的直接链接。
...
`/api/version` 端点会读取最新的 GitHub Release 标签并在边缘缓存 5 分钟（`s-maxage=300`，重新验证期间最多提供 60 秒的陈旧结果，GitHub 出错时最多提供 1 小时），因此版本检查不会在每次请求时都命中 GitHub API。
