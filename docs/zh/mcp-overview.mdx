...
<Info>
**Pro 和 API 套餐均可通过 OAuth 接入 —— 无需 API key。** Pro 订阅者只需在授权页点击 _"Sign in with WorldMonitor Pro"_；API Starter / Business / Enterprise 用户可以同样的方式登录，或者粘贴 `wm_…` key。已确认的**免费账户**同样可以完成 OAuth 流程：其令牌仅限 `free-account` 工具，并按免费额度计量。只有已失效或被停用的付费权益才会在 OAuth 步骤被拒绝并看到 Pro-required 页面。
...
<Note>
如果登录步骤提示无法校验你的订阅 —— 一个 `503` 页面，或来自 grant 端点的 `503 TIER_VERIFICATION_UNAVAILABLE` —— 那是暂时性的，并不代表你的订阅有任何问题。请等待响应中标注的 `Retry-After`，然后从你的客户端重新发起连接；该授权会话是一次性的，因此需要重新发起而不是刷新页面。付费权益已过期或被停用的已确认账户会收到 `403 INSUFFICIENT_TIER`（已确认的**免费**账户不会被拒绝 —— 它以按额度计量的令牌完成流程），而已失效的订阅会收到带 `X-Billing-Verification: subscription_lapsed` 的 `403`。参见[错误处理](/zh/usage-errors)。
</Note>
