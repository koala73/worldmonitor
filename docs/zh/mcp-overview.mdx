...
`get_sources` 不需要 OAuth：它是唯一无需凭据且不消耗每日配额的数据工具。匿名调用使用独立的失败关闭限额：每个 IP 每分钟 10 次。其他所有数据工具仍需订阅。
</Info>
...
|------|------------|------------|-----------------------|-------|
| **Free** | 仅 `get_sources` | 无 | 无每日配额 | 匿名 `get_sources` 调用超过每个 IP 每分钟 10 次时失败关闭。其他数据工具的 OAuth 流程返回 `403 INSUFFICIENT_TIER`。 |
