...
本节大多数工具执行实时 LLM 工作；`get_world_brief` 是缓存支持的例外，返回预计算且有引用依据的仪表板快照。

| 工具 | 描述 | 成本 |
|------|-------------|------|
| `get_world_brief` | 与仪表板共享的预计算、有引用依据的世界简报。为兼容保留 `geo_context`，但不会缩小全局快照范围。 | 缓存 |
...
| `get_world_brief` | `news:insights:v1`（预填充 Redis key；无 REST 代理） |
