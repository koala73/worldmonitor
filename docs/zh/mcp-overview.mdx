...
连接始终需要凭据：传输端点会对未认证请求返回 `401` 与 `WWW-Authenticate` 质询，这正是触发客户端登录的信号。免费账户即可；登录后 `get_sources` 不产生任何费用（无需订阅，也不消耗每日配额）。其他数据工具需要具备相应权益的绑定用户凭据；标记为 `free-account` 的缓存工具可使用已确认免费账户的小额额度，标记为 `subscription` 的实时获取工具需要 Pro。
</Info>
...
**传输端点在连接时即发出质询。** 对 `https://worldmonitor.app/mcp` 的任何未携带凭据的请求——包括 `initialize`、`tools/list` 及其他目录方法——都会得到 `401`、JSON-RPC `-32001`，以及指向该路径[受保护资源文档](/zh/api-oauth#发现端点)的 `WWW-Authenticate` 质询，并回显请求 id。这第一个 `401` 正是 MCP 客户端得知需要登录的方式，托管连接器（Cursor Agents、Grok、ChatGPT）会在首次探测服务器时记录它：若该探测得到 `200`，它们会把服务器标记为无需登录，之后付费工具拒绝调用时便没有可供跳转的授权服务器。工具目录本身并非机密——它已发布在静态[服务器卡片](https://worldmonitor.app/.well-known/mcp/server-card.json)中，智能体就绪度扫描器仍可在机器发现别名 `/.well-known/mcp` 与 `/.well-known/mcp.json` 上完成匿名握手，限速为每个客户端 IP 每分钟 60 次。这些别名面向扫描器与爬虫；MCP 客户端请指向 `/mcp`。每个 `tools/list` 与 `describe_tool` 条目都带有 `_meta["worldmonitor/access"]`：`"free"` 表示无需订阅且不计配额，`"free-account"` 表示已登录的免费账户可用（缓存类数据调用消耗其额度；`describe_tool` 不消耗），`"subscription"` 表示仅限 Pro。资源模板与其对应工具使用相同标记。携带但无效的凭据会返回 `401`，绝不会被静默降级为匿名。
...
|------|------------|------------|-----------------------|------------|-------|
| **Free** | 登录免费账户后可用 `get_sources`（不计配额）与 `free-account`（缓存读取）工具 | OAuth（已确认的免费账户可完成流程）或现有绑定用户凭据 | **每 UTC 日 5 次调用和 3 个空闲间隔请求窗口** | 匿名上限仅适用于发现别名：`get_sources` 每 IP 每分钟 10 次，发现方法每 IP 每分钟 60 次 | 令牌仅限 `free-account` 工具；`subscription` 工具返回升级拒绝。 |
...
资源分为三个访问类别。**公开具体资源**出现在每个 `resources/list` 中，无需订阅且不计配额（匿名发现别名向扫描器提供的也是这些资源）；**账户资源**只向已认证、绑定用户的凭据公开，但读取时不消耗配额；**URI 模板**是参数化且承载数据的资源，其访问要求由 `_meta["worldmonitor/access"]` 声明为 `free-account` 或 `subscription`，并按调用者适用的免费额度或订阅配额计量。

`resources/list` —— 具体、无需订阅、不计配额：
