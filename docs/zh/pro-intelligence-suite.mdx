...
它们共享只读契约，而不是同一条流水线。种子节奏与对外表面按数据集而不同：实物溢价每日刷新并驱动背离指数；矿产产量每 60 天；国防工业基础的世界银行快照每 10 天、SIPRI 供应份额每 14 天；人口结构每 20 天；粮食库存每 30 天；供应脆弱性与五因子记分卡按日投影衍生快照；韧性指标在请求时从分数追踪构建。每次读取都由 sebuf RPC 提供。多数路由还有 MCP 封装，但并非全部——`list-vulnerability-rankings` 仅有 REST。它们都不接受写入。所有数据集都携带明确的来源信息和明确的不可用状态，因此缺失的数值永远不必被读作零。
