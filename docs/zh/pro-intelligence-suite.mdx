---
title: "Pro 情报套件"
description: "仅限 Pro 的衍生数据集——实物金属溢价与背离、矿产集中度、国防工业基础、人口结构、粮食库存、韧性指标、供应脆弱性与五因子记分卡——附每个数据集的 REST 路由与 MCP 工具。"
---

一组衍生数据集位于 Pro 层之后。每一个都回答原始数据源无法回答的问题：不是"黄金价格是多少"，而是"当前黄金的实物溢价是否异常"；不是"钴在哪里"，而是"谁在精炼它，集中度有多高"。

它们共享同一结构。每日种子程序将有界快照写入 Redis，sebuf RPC 提供该快照，MCP 工具为智能体封装该 RPC。它们都不接受写入。所有数据集都携带明确的来源信息和明确的不可用状态，因此缺失的数值永远不必被读作零。
...
未认证的调用返回 `401` 及 `{"error":"Pro authentication required"}`。已登录的免费账户返回 `403`，其中 `error.data.reason = "upgrade-required"` 并附 `upgradeUrl`。两者都是最终答复而非临时故障——不要重试。携带 `X-Billing-Verification` 的 `503` **可以**重试；请遵守 `Retry-After`。参见[使用错误](/zh/usage-errors)。
...
状态字段至关重要。有效历史点少于 60 个时分类器拒绝评级：`state` 变为 `PHYSICAL_DIVERGENCE_STATE_INSUFFICIENT_HISTORY`，`reason` 说明缺口，指数为 null。请在读取任何数值字段之前先读 `state`。
...
`supplierHhi` 基于完整的 TIV 分母计算，而 `suppliers` 仅列出能映射到 ISO-2 国家的行。`supplierMappingCoverage` 是已映射的份额——在将所列供应国视为全貌之前请先读取它。`supplierRetained: true` 表示当前 SIPRI 请求失败，保留了此前的进口国记录而非丢弃。
...
USDA PSD 谷物期末库存、产量、消费量以及库存消费比，按国家与商品划分。`country_code=WORLD` 返回全球平衡表。`commodity` 接受 `wheat`、`corn`、`rice`、`soybeans`、`barley`、`palmOil`。
...
国家韧性评分之下的可解释层：全部 72 项注册指标及其归一化得分、观测或插补状态、运行时权重、按维度对账的贡献值、观测时效与来源出处。
...
| `GET /api/supply-chain/v1/get-chokepoint-dependencies?chokepoint_id=hormuz_strait` | `get_chokepoint_dependencies` | 对某一咽喉要道依赖度最高的国家与商品 |
...
| 路由 | MCP 工具 | 返回内容 |
| --- | --- | --- |
| `GET /api/scorecard/v1/get-five-factor-scorecard?country_code=DE` | `get_five_factor_scorecard` | 单一国家，完整证据账 |
