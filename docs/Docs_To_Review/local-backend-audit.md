...
|---|---|---|---|---|---|---|
| P1 | LiveNewsPanel | `src/services/live-video/session.ts` | `/api/youtube-embed` (sidecar player), `/api/youtube/live` (names a video added in channel management) | `src-tauri/sidecar/local-api-server.mjs`, `api/youtube/live.js` | Fully local | ✅ Each channel plays its broadcaster HLS stream or verified YouTube streams through the local sidecar player, in catalog order, and says why when none is live. |
...
| P1 | Map layers (conflicts, outages, AIS, military flights) | `src/services/conflicts.ts`, `src/services/outages.ts`, `src/services/ais.ts`, `src/services/military-flights.ts` | `/api/acled-conflict`, `/api/cloudflare-outages`, `/api/ais-snapshot`, desktop-product-only `/api/opensky` | `api/acled-conflict.js`, `api/cloudflare-outages.js`, `api/ais-snapshot.js`, `api/opensky.js` | Requires product authentication | ✅ Explicit fallback: unavailable feeds are disabled while map rendering remains active for local/static layers. |
...
| P3 | OpenSky relay fallback path | `src/services/military-flights.ts` | Desktop-product-only `/api/opensky` | `api/opensky.js` | Requires Tauri origin and enterprise product key | ✅ Relay fallback documented; no hard failure when relay is unavailable. |
