...
1. Claude Desktop reads `claude_desktop_config.json`, sees the WorldMonitor server, and on first use POSTs `initialize` to `https://worldmonitor.app/mcp`. The server responds with its capabilities, the negotiated protocol version (default `2025-03-26`; the static server card advertises `2025-06-18`, gated at handshake by `MCP_PROTOCOL_FLOOR_2025_06_18`), and a session-level `instructions` string that tells the model about the universal `jmespath` argument. See [protocol negotiation](/mcp-server#protocol-negotiation) for the rollout details.
