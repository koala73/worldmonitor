...
- **Grounded world brief** (precomputed cache, refreshed by the insights seeder) — `get_world_brief`.
...
3. When you ask a question that needs live data, Claude picks a tool, calls `tools/call`, and inlines the response into its reply. Cache tools and `get_world_brief` return in under a second; live LLM-backed tools (`analyze_situation`, `get_country_brief`, etc.) take 1–4 s.
