...
Connecting requires signing in: the server challenges an unauthenticated client on its first request, which is what makes your MCP client open the sign-in window. A free WorldMonitor account is enough to connect. With it, `get_sources` costs nothing (no subscription, no daily quota) and a small allowance covers the cache-backed tools. Every other data tool is subscription-gated. For those tools, you need one of:
...
- **With an API key, pass it as a header.** Use the single-argument form `X-WorldMonitor-Key:wm_…` with **no space after the colon** — it sidesteps client/`npx` argument-escaping bugs with spaces inside `args` (documented by `mcp-remote` for Cursor and Claude Desktop on Windows). A request that carries the key is never challenged.
- **Prefer OAuth?** Omit `--header`. The server answers `mcp-remote`'s first request with the `401` challenge, which opens its browser consent flow at connect time — the path `mcp-remote` handles most reliably (dynamic client registration works on both `worldmonitor.app` and `www.worldmonitor.app`). A free account can complete it; subscription tools then depend on your plan.
