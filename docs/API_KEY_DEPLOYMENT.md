...
- **Trusted browser origins** (`worldmonitor.app`, Vercel previews, localhost dev) — allowed without a key only with explicit trusted `Origin` and browser `Sec-Fetch-*` metadata
