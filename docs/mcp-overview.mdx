...
Connecting always requires a credential: the transport answers an unauthenticated request with `401` and the `WWW-Authenticate` challenge, which is what starts your client's sign-in. A free account is enough, and with it `get_sources` costs nothing (no subscription, no daily quota). Other data tools require a user-bound credential with the matching entitlement. Tools marked `free-account` (direct cache reads) can use the small confirmed-free allowance; tools marked `subscription` (everything with server-side execute logic — live fetches, plus a few local-compute tools) require Pro.
</Info>
...
**The transport challenges at connect time.** A request to `https://worldmonitor.app/mcp` that presents no credential — `initialize`, `tools/list` and the other catalog methods included — is answered with `401`, JSON-RPC `-32001`, and a `WWW-Authenticate` challenge naming the [protected-resource document](/api-oauth#discovery) for that path, with the request id echoed. That first `401` is how an MCP client learns it must sign in, and hosted connectors (Cursor Agents, Grok, ChatGPT) record it when they first probe the server: answer that probe with `200` and they mark the server as needing no sign-in, then have no authorization server to send you to when a paid tool later refuses the call. The catalog itself is not secret — it is published in the static [server card](https://worldmonitor.app/.well-known/mcp/server-card.json), and agent-readiness scanners can still complete an anonymous handshake on the machine-discovery aliases `/.well-known/mcp` and `/.well-known/mcp.json`, rate-limited to 60 requests/minute per client IP. Those aliases are for scanners and crawlers; point MCP clients at `/mcp`. Every `tools/list` and `describe_tool` entry carries `_meta["worldmonitor/access"]`: `"free"` means no subscription and no quota, `"free-account"` means available to a signed-in free account (cache-backed data calls spend its allowance; `describe_tool` does not), and `"subscription"` means Pro-only. Resource templates carry the same marker as their backing tool. A credential that is present but invalid returns `401`, never a silent anonymous downgrade.
...
|------|------------|------------|-----------------------|------------------|-------|
| **Free** | `get_sources` (no quota) and `free-account` (cache-read) tools, with a signed-in free account | OAuth (confirmed free accounts complete the flow) or an existing user-bound credential | **5 calls and 3 idle-gap request windows / UTC day** | Anonymous ceilings apply on the discovery aliases only: 10/minute/IP on `get_sources`, 60/minute/IP on discovery | The token is restricted to `free-account` tools; `subscription` tools return the upgrade denial. |
...
| Tool | Description |
|------|-------------|
| `get_sources` | Live source inventory (providers + outlets) with tiers and provenance. The sole data tool that needs no subscription and spends no quota. |
...
Resources have three access classes. **Public concrete resources** appear in every `resources/list`, need no subscription, and are quota-free (they are also what the anonymous discovery aliases serve to scanners). **The account allowance resource** appears in `resources/list` only for a user-bound authenticated client and is also quota-free. **URI templates** (parameterised, data-bearing) are surfaced by `resources/templates/list`; their `_meta["worldmonitor/access"]` marker states whether a signed-in free account or a Pro subscription can read each concrete instantiation.

`resources/list` — concrete, no subscription needed, quota-free:
...
`resources/read` on a `ui://` URI returns the self-contained HTML view. Unlike the data resources, a `ui://` read is **public and quota-exempt** — the template carries no data and spends no upstream call, so a connected host can preload it (and an agent-readiness scanner can fetch it from the anonymous discovery aliases) without touching the Pro daily cap. The view is fully self-contained (no external assets) and communicates with the host over the standard MCP Apps `postMessage` bridge (`ui/initialize` → `ui/notifications/tool-result` → `ui/notifications/size-changed`).
