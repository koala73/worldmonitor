...
| `https://worldmonitor.app/docs/mcp` | Separate anonymous **documentation** MCP server (docs search + read-only docs filesystem) — card at `/.well-known/mcp/docs-server-card.json`; see [Agent Discovery](/agent-discovery#agent-front-doors) |
...
OAuth bearer requests re-check the resolved MCP token, user binding, and entitlement before dispatch. A provider-confirmed lapse removes paid capability but retains the OAuth identity on the restricted, allowance-metered `free-account` path; an expired or disabled paid entitlement without that confirmed lapse, or another genuinely insufficient non-free state, is denied on the next request. Dashboard-issued `X-WorldMonitor-Key: wm_…` requests validate the key owner and active entitlement, then use the same per-user minute bucket and 50/day default as the OAuth path. Legacy deployment-allowlisted operator keys use a per-key minute bucket and skip the daily reservation.
...
|------|------------|------------|-----------------------|-------|
| **Free** | Anonymous `get_sources`; `free-account` (cache-read) tools with a user-bound credential | OAuth (confirmed free accounts complete the flow) or an existing user-bound credential | **5 calls and 3 idle-gap request windows / UTC day** | The token is restricted to `free-account` tools; `subscription` tools return the upgrade denial. Anonymous `get_sources` fails closed above 10/minute/IP. |
...
WebMCP and a ChatGPT custom app are different integrations. Typing “use WebMCP from worldmonitor.app” into a normal conversation does not attach either one: ChatGPT is not running as the browser agent in the WorldMonitor tab, and the conversation has not been granted access to the hosted MCP server.

Connect the hosted server from ChatGPT on the web:

1. Open **Settings → Apps & Connectors → Advanced settings** and enable **Developer mode**. Workspace availability and the ability to add apps can depend on the ChatGPT plan and administrator policy.
2. Choose **Create app**, name it `WorldMonitor`, and set the MCP server URL to `https://worldmonitor.app/mcp`.
3. Complete the WorldMonitor sign-in and consent flow when ChatGPT requests authorization.
4. Start a new conversation, open its app/tool picker, and enable the `WorldMonitor` app before asking for live WorldMonitor data. Existing conversations do not automatically acquire a newly connected app.

Configure the app on ChatGPT web even if you intend to use it later from a mobile client. If **Developer mode**, **Create app**, or the connected app is absent, that ChatGPT account or workspace has not exposed the required custom-app capability; changing WorldMonitor's in-page WebMCP registration cannot make it appear.

For WorldMonitor's tab-bound browser controls instead, open the site in a supported Chrome build and follow the [WebMCP setup](/webmcp#availability).
