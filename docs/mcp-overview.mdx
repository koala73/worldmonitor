...
The server exposes 51 tools. Most are cache-reads over pre-seeded Redis keys (sub-second). The slower, non-cache paths are the six live LLM/external-API tools (`get_world_brief`, `get_country_brief`, `analyze_situation`, `generate_forecasts`, `search_flights`, `search_flight_prices_by_date`) plus the live geo RPC tools (`get_airspace`, `get_maritime_activity`), the bounded canonical procurement proxy (`get_procurement_opportunities`), the canonical China decision-signal RPC (`get_china_decision_signals`), and the derived-analysis tools that recompute shared dashboard engines per request over seeded caches (`get_signal_convergence`, `get_focal_points`, `simulate_infrastructure_cascade`, `get_military_surge`, `get_population_exposure`, `get_alert_digest`, `get_hotspot_escalation`). One (`describe_tool`, added v1.5.0) returns the full uncompressed definition of any other tool — useful when the compressed `tools/list` description is ambiguous; exempt from the Pro daily quota.
...
| `get_temporal_anomalies` | Event counts vs day-of-week/seasonal baselines, z-score severity bands. |
| `get_signal_convergence` | Grid cells where protests, military flights, naval movement, and quakes co-occur. |
| `get_focal_points` | Entities where news coverage and live map signals converge, multi-signal scored. |
| `get_military_surge` | Theater aircraft postures, foreign-presence detections, seeded surge alerts. |
| `get_hotspot_escalation` | 29 curated hotspots scored 1-5 from news, instability, geo convergence, military. |
...
| `simulate_infrastructure_cascade` | What fails downstream when a cable, chokepoint, pipeline, or port is disrupted. |
...
| `get_test_site_seismicity` | USGS earthquakes near nuclear test sites, 0-100 concern scoring + per-site rollup. |
...
| `get_population_exposure` | Estimated people inside the impact radius of quakes, fires, conflict events. |
...
| `get_population_exposure` | `GET /api/displacement/v1/get-population-exposure` |
...
See the [Tool catalog](#tool-catalog) above for the complete list of 51 tools, or the [MCP Tools Reference](/mcp-tools-reference) for per-tool details.

## Prompts and resources

In addition to the 51 tools, WorldMonitor exposes MCP prompts and resources so clients can discover common workflows and address stable data slices without inventing tool plans from scratch.
