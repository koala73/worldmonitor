...
|------|-------------|------|
| `get_world_brief` | Citation-grounded world brief from the same precomputed dashboard snapshot. Legacy `geo_context` is accepted but does not narrow the global snapshot. | Cache |
...
| `get_world_brief` | `news:insights:v1` (seeded Redis key; no REST passthrough) |
