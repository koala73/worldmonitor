...
OAuth is not needed for `get_sources`: it is the sole credential-free, daily-quota-free data tool. Anonymous calls use a separate fail-closed limit of 10 requests/minute/IP. Every other data tool remains subscription-gated.
</Info>
...
|------|------------|------------|-----------------------|-------|
| **Free** | `get_sources` only | None | No daily quota | Anonymous `get_sources` calls fail closed above 10/minute/IP. OAuth for every other data tool returns `403 INSUFFICIENT_TIER`. |
