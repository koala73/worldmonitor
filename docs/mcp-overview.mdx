...
- **`fetch-on-miss`** — may call paid or rate-limited upstreams when cache is cold, or accepts high-cardinality identifiers that are not cache-bundle friendly. Examples: `GET /api/conflict/v1/list-acled-events`, `GET /api/infrastructure/v1/list-service-statuses`, `GET /api/supply-chain/v1/get-critical-minerals`, and `GET /api/aviation/v1/get-flight-status`.
