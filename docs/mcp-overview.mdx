...
All paid tiers share the same MCP server and tool inventory, but they are metered two different ways. **Pro and Pro Business** have no REST allowance, so their MCP calls use a dedicated daily counter — **50** and **250** quota-consuming `tools/call` / `resources/read` calls per UTC day, one unit per call. **API Starter and API Business** have no separate MCP allowance at all: their MCP calls draw on the same daily budget as their REST requests (1,000 and 10,000/day), charged at a per-tool weight of 1 for a cached read, 2 for a live downstream fetch. Both the OAuth and `wm_…` doors resolve the same budget, so the credential you use does not change the cap. Enterprise OAuth can be unlimited, and only legacy operator keys explicitly allowlisted by the deployment skip the daily reservation. Every authenticated context is also protected by the **60 requests/minute** limiter. `get_sources` and the metadata helper `describe_tool` are exempt from the daily quota. Full weight table: [MCP calls against an API plan](/usage-rate-limits#mcp-calls-against-an-api-plan).
...
Need higher-volume scripted access? **API Starter** and **API Business** add a `wm_…` key option plus a much larger daily budget (1,000 and 10,000 units/day) that their MCP calls and REST requests share, alongside the 60/minute/user throttle. A cached MCP read costs 1 unit and a live downstream fetch costs 2, so batch workloads still go further over REST — or contact Enterprise. See [Plans & limits](#plans--limits).
...
| **Enterprise** | ✅ Yes | OAuth **or** `wm_…` key | Unlimited on both doors | Custom SLA available. |
