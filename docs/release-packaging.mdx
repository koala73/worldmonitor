...
---
This guide provides reproducible local packaging steps for the [desktop app](/desktop-app).
...
- OS-native Tauri build prerequisites:
  ...
  - Linux (Debian/Ubuntu, matching `.github/workflows/build-desktop.yml`):
    `libwebkit2gtk-4.1-dev libappindicator3-dev librsvg2-dev patchelf` plus the
    `gstreamer1.0-*` plugin set (`plugins-base`, `plugins-good`, `plugins-bad`,
    `plugins-ugly`, `libav`, `gl`) for the embedded video players
...
```bash
npm run desktop:package -- --os linux
```

The Node sidecar target follows the host architecture (`x86_64-unknown-linux-gnu`
on x64, `aarch64-unknown-linux-gnu` on arm64), or `NODE_TARGET` when set. CI
builds both and `/api/download` serves them as `linux-appimage` and
...
3. Install/launch:
   ...
   - Linux: `chmod +x` the `.AppImage` and launch it.
4. Validate startup:
