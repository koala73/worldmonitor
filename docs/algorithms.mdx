...
Beyond aggregating signals by geography, the system detects meaningful correlations *across* data streams — identifying patterns that no single source would reveal. The correlation engine emits 11 signal types. The `SignalType` union declares 14; the three remaining rows are reserved names with no detector behind them, marked **Not emitted**:
