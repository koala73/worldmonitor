...
- **OREF direct escalation** — any non-empty siren batch raises a critical alert when alerts are enabled and the event is not a duplicate; OREF bypasses the RSS gates and global cooldown
