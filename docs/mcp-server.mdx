...
- Each SSE response carries the JSON-RPC result as a single `message` event with an event `id` (there is no leading empty priming event — an empty-`data:` event still dispatches per the SSE spec, so a strict scanner that parses the first event would fail the handshake). If the client disconnects after receiving that event, it can reconnect by sending `GET /mcp` with `Accept: text/event-stream`, the same `Mcp-Session-Id`, and `Last-Event-ID`; resuming after the delivered event returns an empty stream (nothing follows a single-response), so a dropped-before-delivery client simply re-issues the POST.
