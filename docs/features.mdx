...
- **Per-device by default**: Web builds with `VITE_CLOUD_PREFS_ENABLED=true` sync panel order, visibility, and spans for signed-in users. Map width, height, and side always stay on the device.
