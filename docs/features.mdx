...
A family of derived datasets — physical-metals premiums and divergence, mineral production concentration, defense industrial base, demographics capability, food stocks, resilience indicators, commodity supply vulnerability, and the five-factor country scorecard — requires a Pro subscription on each supported surface. The [Pro Intelligence Suite](/pro-intelligence-suite) documents dashboard, REST, and MCP availability per dataset because not every dataset has every surface. Where a free account meets a supported dashboard surface, the section renders an upgrade card rather than an empty or failed state.

## Interactive Global Map

- **Zoom & Pan** - Smooth navigation with mouse/trackpad gestures
- **Regional Focus** - named preset views for rapid navigation (Global, Americas, Europe, MENA, Asia, Latin America, Africa, Oceania)
- **Layer System** - Toggle visibility of data layers organized by category
...
| Layer | Description |
|-------|-------------|
| **Military Bases** | Global military installations from the authoritative base registry |
...
| Layer | Description |
|-------|-------------|
| **Ships (AIS)** | Live vessel tracking via AIS with chokepoint monitoring and the strategic-port registry* |
...
| **Weather** | Official severe weather alerts (NWS, ECCC, WMO SWIC) |
...
| Layer | Description |
|-------|-------------|
| **Live Webcams** | Registry-backed streams across geopolitical regions with automatic fallback handling |
...
| **Aviation Intelligence** | 6-tab aviation panel (Ops, Flights, Airlines, Tracking, News, Prices) with NOTAM closure detection across the registered airport watchlist |
...
| **Central Banks, BIS & IMF** | Policy-rate and institutional finance signals from central-bank and supranational finance institutions |
...
Live streams from major news networks with channel switching:
...
Each channel tries its sources in order: the broadcaster's own HLS stream where one exists, then verified YouTube live streams played through the official **YouTube IFrame Player API**. A channel counts as live only after the player or the stream playlist confirms it:

| Situation | What the panel shows |
|-----------|----------------------|
| **Connecting** | A "Connecting to…" cover until the stream is confirmed live |
| **Ended, deleted or unembeddable stream** | The next source; when none is left, a card that says why, with Retry, Play next channel and Open on YouTube |
| **Automatic start** (play-all, auto-play, Resume) | Skips a channel that is offline and plays the next one, without changing the saved channel |
| **Player signals blocked** (extensions, strict tracking protection) | The stream keeps playing with a "Can’t confirm this stream is live" note |

Streams are listed in `src/config/live-video-sources.ts`; `npm run live-video:check -- --all` reports any that stopped being live. A daily audit keeps one GitHub issue, "Live video sources: slots needing a replacement", that names each webcam or channel slot with nothing live, where it shows and why; maintainers check a replacement URL with `npm run live-video:check -- <url>` and paste it into that file. YouTube refuses embeds from GitHub's runners, so the audit plays YouTube entries through the residential proxy in the `LIVE_VIDEO_AUDIT_PROXY_URL` secret and stops with an error when that secret is missing. A failure the runner cannot tell apart from its own location or network, such as a region-locked channel refused outside its regions or an HLS playlist that a CDN edge kept serving unchanged, is listed under "Could not verify from the runner" instead of as a slot needing a replacement.

**Idle Detection:**

To conserve resources, playback stops in two cases:

| Trigger | Action |
|---------|--------|
| **Tab hidden** | Stream stops (via Visibility API) |
| **No mouse, keyboard or touch activity** | Stream stops after the idle time chosen in Settings › Media (one hour by default, or never) and shows a notice with Resume |
| **Viewer paused the stream** | Left alone; the idle stop never overrides a manual pause |
...
- **Panels by family**:
  ...
  - **Utilities**: [Airline Intelligence](/panels/airline-intel), [Tech Readiness Index](/panels/tech-readiness), [World Clock](/panels/world-clock), [My Monitors](/panels/monitors), [OREF Sirens](/panels/oref-sirens), [Telegram Intel](/panels/telegram-intel), [X News Accounts](/panels/x-intel), [Financial Stress](/panels/fsi), Webcams, Layoffs
...
Airports in the global registry span the Americas, Europe, Asia-Pacific, MENA, and Africa and are monitored through three independent data sources (FAA ASWS, AviationStack, ICAO NOTAM):
...
Keyboard users reorder with the **⇅** move button at the start of each panel header. The button stays visually hidden until it receives focus. Arrow keys move the panel one slot at a time. When the viewport is at least 900px wide and the map is shown, Page Up moves the panel to the sidebar grid and Page Down moves it to the grid below the map.

### Panel Resizing

Each panel has a resize handle on its bottom edge (height) and its right edge (width):

- **Height**: drag the bottom handle to span 1 to 4 grid rows
- **Width**: drag the right handle to span up to 3 grid columns, limited by how many columns the grid currently has
- **Keyboard**: focus a handle and use the arrow keys, or Home and End for the minimum and maximum
- **Reset**: double-click a handle to restore that panel's default height or width

### Map Width Divider

When the viewport is at least 900px wide, the map and the panel grid sit side by side with a vertical divider between them. Drag the divider to change the map's share of the width, or focus it and press the Left and Right arrow keys to move it in 5% steps. The map keeps a minimum width, and so does the panel column.

### Panel Text Size

The **Panel text size** option on the Settings tab sets text size for every panel. On the Panels tab, the **Text size** selector next to a panel overrides it for that panel after you click Save. **Use global** returns the panel to the global size.

### Reset Layout

**Reset Layout** on the Panels tab of Settings clears panel order, row and column spans, per-panel text sizes, map height, map width, and map side, then reloads the page.
...
- **LocalStorage**: Panel order, visibility, collapsed state, row and column spans, per-panel text size, map width, map height, and map side
- **Automatic save**: Drag, keyboard, and resize changes persist immediately; Panels tab changes persist when you click Save
- **Per-device by default**: Web builds with `VITE_CLOUD_PREFS_ENABLED=true` sync panel order, visibility, spans, and per-panel text size for signed-in users. Collapsed state, map width, map height, and map side always stay on the device.
