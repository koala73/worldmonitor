...
- **A bad or over-long expression returns `HTTP 400`** with the same `{ _jmespath_error, original_keys }` body (the MCP transport soft-fails inside a 200 result instead). The ≤ 1024-byte expression limit is identical; there is no fixed output cap, because the un-projected body was already going to be sent in full.
