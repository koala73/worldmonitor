...
7. **Cleanup PR** (PR 2 follow-up): remove `_countRealtimeAllRules` and `_migrateRealtimeAllPage` and the driver scripts. Delete the Convex env var: `npx convex env remove MIGRATION_ADMIN_SECRET`. The migration functions exist as a temporary admin surface — they should not remain in the codebase post-migration.
