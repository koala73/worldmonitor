...
- **Connecting:** every request needs a credential. An unauthenticated request, `initialize` and `tools/list` included, gets `401` with a `WWW-Authenticate` challenge, which is what starts your client's OAuth sign-in. A free account is enough to connect.
- **`get_sources` via `tools/call`:** no subscription and no daily quota. Its `tools/list` and server-card entries carry `_meta["worldmonitor/access"]: "free"`.
