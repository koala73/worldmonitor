/**
 * Cyber service handler -- implements the generated CyberServiceHandler
 * interface with 1 RPC aggregating five upstream threat intelligence sources:
 ...
 *
 * Consolidates legacy edge function: api/cyber-threats.js
 *
...
const THREAT_TYPE_MAP: Record<string, CyberThreatType> = {
...
const SOURCE_MAP: Record<string, CyberThreatSource> = {
...
const SEVERITY_MAP: Record<string, CriticalityLevel> = {
...
async function hydrateThreatCoordinates(threats: RawThreat[]): Promise<RawThreat[]> {
...
function dedupeThreats(threats: RawThreat[]): RawThreat[] {
...
function toProtoCyberThreat(raw: RawThreat): CyberThreat {
  return {
    id: raw.id,
  ...
  async listCyberThreats(
    ...
    try {
      ...
      let results = hydrated
        .filter((t) => hasValidCoordinates(t.lat, t.lon))
        .sort((a, b) => {
