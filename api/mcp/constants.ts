...
export const SERVER_INSTRUCTIONS = [
  ...
  '',
  `get_sources is the sole credential-free data tool and consumes no daily quota. It has a separate fail-closed ceiling of 10 unauthenticated calls/minute/IP. Signed-in accounts without a subscription get a free taste of the CACHED-data tools (3 request windows/day, 5 calls/day); live-fetch tools stay Pro-only and answer -32002/403 reason=upgrade-required. Every denial carries \`error.data\` = {reason, nextStep, upgradeUrl}: -32001/401 reason=no-account (re-authenticate), -32029/429 reason=allowance-exhausted (wait for the next UTC day), -32002/403 reason=upgrade-required or lapsed-subscription (do NOT re-authenticate; subscribe). In tools/list, get_sources carries \`_meta["worldmonitor/access"] = "free"\`; omission of that marker means subscription-gated. Upgrade: ${MCP_UPGRADE_URL}.`,
  '',
